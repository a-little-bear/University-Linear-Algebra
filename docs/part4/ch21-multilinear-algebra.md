...
1. **[概念] 线性泛函和对偶空间的概念为什么在物理中非常重要？**
   ??? success "参考答案"
       在物理中，向量通常代表客观的“箭头”（如位移、速度），而线性泛函代表对这些“箭头”的测量仪器（如等高线、等压面、动量）。对偶空间将“测量仪器”本身也构成了一个空间。

2. **[维数] 设 $V$ 是 $m$ 维，$W$ 是 $n$ 维，张量积 $V \otimes W$ 的维数是多少？**
   ??? success "参考答案"
       维数是 $m \times n$。它的基是两空间基向量的两两组合 $\mathbf{e}_i \otimes \mathbf{f}_j$。

3. **[性质] 为什么通常情况下张量不是“简单张量”（即不能写成单个 $\mathbf{v} \otimes \mathbf{w}$）？**
   ??? success "参考答案"
       因为简单张量只占据了张量空间 $V \otimes W$ 中一个非常小的代数簇（类似于矩阵中秩为 1 的矩阵）。大部分张量是多个简单张量的线性组合，在量子力学中这对应着“纠缠态”。

4. **[外代数] 在外代数中，$\mathbf{v} \wedge \mathbf{v} = 0$ 的几何意义是什么？**
   ??? success "参考答案"
       它意味着一个向量与自身张成的平行四边形的面积（或高维体积）为零，反映了外积的反对称性质。

5. **[行列式] 证明 $n$ 维向量空间 $V$ 上的 $n$ 阶外幂 $\Lambda^n(V)$ 是 1 维的，这如何引出行列式？**
   ??? success "参考答案"
       $\Lambda^n(V)$ 只有唯一的基 $\mathbf{e}_1 \wedge \dots \wedge \mathbf{e}_n$。任何 $n$ 个向量的外积必然是这个基向量的标量倍，这个缩放系数恰好就是这些向量构成矩阵的行列式。

6. **[约定] 爱因斯坦求和约定 $a^i b_i$ 在线性代数中代表什么运算？**
   ??? success "参考答案"
       代表内积或泛函在向量上的作用（当一个指标在一上一下同时出现时，自动对其求和，如 $\sum_i a^i b_i$）。

7. **[分解] CP 分解与 Tucker 分解的主要区别是什么？**
   ??? success "参考答案"
       CP 分解将张量直接分解为最小数目的秩一简单张量的和（其项数即为张量秩）。Tucker 分解则提取出一个稠密的“核心张量”以及每个维度上的投影矩阵，类似于高阶的主成分分析。

8. **[难点] 为什么说计算张量的秩是 NP-hard 的？**
   ??? success "参考答案"
       对于矩阵（二阶张量），我们可以通过 SVD 或高斯消元轻松确定秩。但对于三阶以上的张量，求将其分解为最少个秩一张量之和的问题，已被证明在计算复杂性理论上是极难的（NP-hard）。而且其在实数域和复数域上的秩可能不同。

9. **[同构] 解释自然同构 $V \otimes W^* \cong \operatorname{Hom}(W, V)$。**
   ??? success "参考答案"
       一个简单张量 $\mathbf{v} \otimes f$ 可以自然地视为一个线性映射：输入 $\mathbf{w}$，先用泛函 $f$ 测量出标量 $f(\mathbf{w})$，再乘以向量 $\mathbf{v}$。这展示了张量积如何编码所有的线性映射（矩阵）。

10. **[爱因斯坦思考题] 为什么广义相对论必须使用张量方程（如 $R_{\mu\nu} - \frac{1}{2}R g_{\mu\nu} = 8\pi G T_{\mu\nu}$）来表述？**
    ??? success "参考答案"
        因为爱因斯坦的“广义协变性原理”要求物理定律在任何坐标系（观察者）下都必须具有相同的形式。张量作为一个绝对的几何客体，其各个分量在基变换时所发生的逆变与协变相乘后，总能完美抵消坐标系的偏见。这使得张量方程成为宇宙客观真理的唯一合格载体。

## 本章小结

本章将线性映射的概念推广到多变元情形，建立了几何与代数的统一语言——张量理论：

1. **对偶空间**：将线性泛函视为独立的空间 $V^*$，引出了双对偶的自然同构。
2. **多线性映射与张量积**：通过泛性质严格定义了张量积 $V \otimes W$，揭示了它如何将所有的双线性问题转化为单线性问题。
3. **分量与爱因斯坦约定**：用上下指标区分逆变和协变，推导了张量在坐标变换下必须遵循的代数规则。
4. **外代数与行列式**：通过反对称化算子定义了楔积 $\wedge$，优雅地证明了行列式的多线性、反对称和体积缩放的几何本质。
5. **张量分解与量子纠缠**：超越了传统的矩阵分解（SVD），简述了高阶张量的 CP 和 Tucker 分解，以及它们在表示量子纠缠态和高维数据挖掘中的前沿应用。
