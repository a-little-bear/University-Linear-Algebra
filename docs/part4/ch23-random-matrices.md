...
1. **[系综] GOE, GUE, GSE 三大高斯系综分别对应什么类型的随机矩阵？**
   ??? success "参考答案"
       GOE（正交）对应实对称阵，GUE（酉）对应复厄米特阵，GSE（辛）对应四元数自双对偶阵。它们分别具有正交群、酉群和辛群下的相似变换不变性。

2. **[半圆律] 为什么 Wigner 矩阵的特征值经验分布在极限下呈现半圆律而不是高斯分布（尽管其矩阵元素是独立的高斯分布）？**
   ??? success "参考答案"
       因为矩阵的特征值不是矩阵元素的简单线性叠加。特征值之间存在强烈的非线性相互作用（排斥效应），这打破了传统中心极限定理所要求的独立性假设，从而塑造了受限的半圆形“能带”密度。

3. **[MP律] Marchenko-Pastur 律中的参数 $c = p/n$ 代表什么？当 $c > 1$ 时谱分布在原点有何特征？**
   ??? success "参考答案"
       $c$ 代表数据维度（特征数）与样本数量的比例。当 $c > 1$ 时（高维少样本），协方差矩阵必然是不满秩的。因此其经验谱分布在零点必定会有一个权重为 $1 - 1/c$ 的狄拉克 $\delta$ 脉冲（集中了大量零特征值）。

4. **[特征值间距] 随机矩阵的 Wigner-Dyson 分布与独立事件的泊松分布（Poisson）在能级间距上有什么根本区别？**
   ??? success "参考答案"
       泊松分布在间距 $s \to 0$ 时概率密度最大（独立事件容易扎堆），而 Wigner-Dyson 分布在 $s \to 0$ 时概率密度趋于零（即存在**能级排斥**现象，特征值不愿彼此靠近）。

5. **[Tracy-Widom] 传统中心极限定理描述了大量独立变量和的涨落，而 Tracy-Widom 分布描述了什么物理量的涨落？**
   ??? success "参考答案"
       描述了随机矩阵**最大特征值**（或谱边缘的极端特征值）在其极限分布边缘周边的微观非高斯涨落规律。

6. **[BBP相变] 在尖峰协方差（Spiked Covariance）模型中，如果真实信号的强度低于某个阈值，最大特征值会有什么表现？**
   ??? success "参考答案"
       它将被完全“淹没”在主体噪声（如 MP 律的主体支撑带）之中，使得我们无法通过观察最大的特征值来提取信号；只有当信号强度大于临界阈值时，最大特征值才会“弹出”连续谱的主体，这就是著名的 BBP 相变。

7. **[Stieltjes变换] Stieltjes 变换在随机矩阵理论中起到了什么类似于概率论中特征函数的作用？**
   ??? success "参考答案"
       它将复杂的测度弱收敛问题转化为了复平面上解析函数的逐点收敛问题。由于矩阵的预解式（Resolvent）的迹正好是经验谱的 Stieltjes 变换，我们可以利用代数方程求出其极限解析式，进而通过留数定理或反演公式唯一恢复出经验谱的密度函数。

8. **[自由概率] 经典概率论研究独立随机变量的和的分布使用普通卷积，自由概率论使用什么工具来计算非对易的渐进独立随机矩阵之和的极限谱分布？**
   ??? success "参考答案"
       使用 Voiculescu 的自由加法卷积（Free Convolution $\boxplus$）以及与之对应的加性 $R$-变换。

9. **[物理渊源] 随机矩阵理论最初是由物理学家维格纳（Eugene Wigner）为了解释什么现象而引入的？**
   ??? success "参考答案"
       为了解释重原子核复杂的能级间距统计规律。维格纳猜测，对于极度复杂的量子多体系统，其哈密顿量矩阵的微观细节已不再重要，可以被视为完全随机的，只需满足宏观的物理对称性（如时间反演对称性），就能刻画其能级的统计特征。

10. **[爱因斯坦思考题] 爱因斯坦曾说“上帝不掷骰子”，但在随机矩阵理论中，我们用纯粹的随机性却推导出了极端普适、确定的宏观定律（如半圆律、Tracy-Widom分布等 Universality 现象）。这在统计物理上启示了什么深层真理？**
    ??? success "参考答案"
        它揭示了自然界的“普适性”（Universality）：在包含无穷多自由度的强耦合巨型系统中，微观个体层面的具体规则（不管骰子是怎么掷的，是高斯分布还是抛硬币）最终都会被高维极限定理所完全“抹平”。在这里，局部的纯粹随机性不仅没有导致彻底的无序，反而通过大数维度的约束突现（Emergence），产生出了甚至比简单确定性动力学更加精确可控的“统计确定性”。
