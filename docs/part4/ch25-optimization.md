...
1. **[线性规划] 单纯形法（Simplex Method）在几何上的搜索路径是怎样的？**
   ??? success "参考答案"
       沿着高维凸多面体（可行域）的棱进行一维搜索，每次迭代从一个顶点平滑滑动到相邻的使目标函数最速下降的顶点，直至到达全局最优顶点所在的局部盆地。

2. **[最小二乘] Tikhonov 正则化（岭回归，Ridge Regression）的目标函数是什么？它对原矩阵 $A^TA$ 的特征值做了什么改变？**
   ??? success "参考答案"
       目标是 $\min \|A\mathbf{x} - \mathbf{b}\|_2^2 + \lambda \|\mathbf{x}\|_2^2$。它将极度病态（甚至奇异）的 $A^TA$ 强行修改为了 $A^TA + \lambda I$，也就是将所有特征值硬性拔高了 $\lambda$，从而根除了矩阵不可逆的可能，使得逆变换极其稳定。

3. **[对偶性] 半定规划（SDP）的强对偶性在什么条件下保证成立？**
   ??? success "参考答案"
       需要满足斯莱特条件（Slater's condition），即存在一个严格可行的相对内点（在半定规划中，意味着存在某个可行的矩阵变量不仅半正定，而且是严格**正定**的）。

4. **[稀疏性] 为什么 $\ell_1$ 范数最小化（Lasso）能够天然产生稀疏解（Sparse Solution），而 $\ell_2$ 范数不能？**
   ??? success "参考答案"
       $\ell_1$ 范数的单位等高面是带有“尖锐突角”的菱形多面体。在空间中膨胀与线性约束超平面首次相切时，绝大概率会撞击在这些尖角上（尖角上的坐标点大量分量直接为 0）。$\ell_2$ 则是各向同性的圆球，相切点几乎永远不在坐标轴上，导致所有分量都“非零但微小”。

5. **[低秩逼近] 矩阵的哪种范数是矩阵“秩”（$\operatorname{rank}$，即 $\ell_0$ 范数的非凸推广）的最佳凸包络（Convex Envelope）？**
   ??? success "参考答案"
       核范数（Nuclear Norm），即矩阵所有奇异值绝对值之和。它在矩阵补全和鲁棒 PCA 中彻底取代了 NP 难的秩函数，使降维问题蜕变为计算上多项式可解的凸优化。

6. **[PCA] 鲁棒主成分分析（Robust PCA）旨在将现实世界中受污染的数据矩阵分解为哪两个具有鲜明代数特征的部分？**
   ??? success "参考答案"
       一个**低秩**矩阵（代表稳定的主要背景、主特征提取）和一个**极其稀疏**的矩阵（代表少数像素点上发生的大幅度突变、前景对象或恶劣随机噪声）。

7. **[降维] Johnson-Lindenstrauss (JL) 引理指出，将千万维的点集随机投影到极低维子空间时，什么性质竟然能大概率得以近乎完美的保持？**
   ??? success "参考答案"
       任意两点之间的欧氏距离（成对距离）会以极大的概率保持在极小的相对误差 $\epsilon$ 范围内，这给哈希降维和近似最近邻搜索（ANN）赋予了降维打击般的理论支柱。

8. **[特征值] Courant-Fischer 极小极大定理为图的拉普拉斯矩阵的第二小特征值（Fiedler 值）赋予了什么样的代数优化意义？**
   ??? success "参考答案"
       在所有正交于全 1 向量的单位实向量中，使得二次型 $\mathbf{x}^T L \mathbf{x}$ 最小化的极小值下限。它精确度量了将图切割为两块部分的最小代数连通度，是谱聚类的引擎。

9. **[LMI] 在现代控制理论中，线性矩阵不等式（LMI）定义的可行域被称为一个什么样的集合？**
   ??? success "参考答案"
       被称为一个谱面体（Spectrahedron），因为它是无穷维的半定锥与有穷维的仿射平面的截面交集，这严格保证了该可行域的整体凸性，从而保障了 SDP 求解的全局收敛。

10. **[爱因斯坦思考题] 压缩感知（Compressed Sensing）惊人地打破了传统的香农-奈奎斯特采样定律（必须两倍于最高频率采样才能无损恢复）。这种“无中生有、欠定求精”的魔法，从线性代数的纯粹视角来看，是依赖了宇宙的什么先验公理？**
    ??? success "参考答案"
        它绝对依赖了宇宙万物（真实信号）在某种观测基底下的**极端稀疏性（Sparsity）**。如果信号是不可预测的热力学白噪声，则永远无法压缩还原；但真实的物理实体信号（如星系的光谱、黑洞的照片的边缘），总是被极少数的核心主频或特征主导。压缩感知正是利用了 $\ell_1$ 凸优化的超凡尖角过滤能力，在欠定方程浩如烟海的无穷多解空间中，如手术刀般精确揪出了那个最遵循“奥卡姆剃刀”原理（拥有最多 0 分量）的唯一真实物理态。

---

## 本章小结

本章全景展示了线性代数与泛函分析在现代最优化理论、算法及高维统计中的主宰地位：

1. **线性规划**：单纯形法的运转机制本质上是线性方程组在非负象限锥边沿上求解的列重组迭代过程，基本可行解一一对应于基底的选取。
2. **最小二乘极值与正则化**：利用正规方程、QR 分解或 SVD 深究了超定问题，并确立了 Tikhonov 正则化（岭回归）通过条件数截断来抑制高频噪声灾难性放大的原理。
3. **半定规划（SDP）**：将传统线性规划中的向量正象限 $\mathbb{R}_+^n$ 推广升维到了正半定矩阵锥空间 $\mathbb{S}_+^n$，建立了以 LMI 为约束的强对偶性极值理论。
4. **矩阵补全（Matrix Completion）**：首次引入利用极度凸的核范数松弛机制，将 NP-hard 的离散秩极小化问题化解为可凸性求解的连续降维问题。
5. **压缩感知（Compressed Sensing）**：利用线性代数中高瘦矩阵特殊的受限等距性质（RIP条件），给出了 $\ell_1$ 最小化能极高概率实现欠定方程精确恢复的充要条件。
6. **鲁棒 PCA**：使用奇异值软阈值算法将脏数据分解，证明了低秩与稀疏成分在凸优化眼中的互相独立性。
7. **高维降维几何**：JL 随机投影引理和经典的 Eckart-Young-Mirsky 定理联手为海量维度数据的欧氏距离约减提供了深邃的概率保证。
8. **极值工具箱**：Courant-Fischer 极小极大定理和 Weyl 不等式彻底确立了特征值作为流形上最优化结果的绝对地位，并为整个扰动分析确立了数学疆界。
