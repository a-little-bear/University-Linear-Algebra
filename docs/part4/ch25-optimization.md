...
****：单纯形法的运转机制本质上是线性方程组在非负象限锥边沿上求解的列重组迭代过程，基本可行解一一对应于基底的选取。

****：利用正规方程、QR 分解或 SVD 深究了超定问题，并确立了 Tikhonov 正则化（岭回归）通过条件数截断来抑制高频噪声灾难性放大的原理。

****：将传统线性规划中的向量正象限 $\mathbb{R}_+^n$ 推广升维到了正半定矩阵锥空间 $\mathbb{S}_+^n$，建立了以 LMI 为约束的强对偶性极值理论。

****：首次引入利用极度凸的核范数松弛机制，将 NP-hard 的离散秩极小化问题化解为可凸性求解的连续降维问题。

****：利用线性代数中高瘦矩阵特殊的受限等距性质（RIP条件），给出了 $\ell_1$ 最小化能极高概率实现欠定方程精确恢复的充要条件。

****：使用奇异值软阈值算法将脏数据分解，证明了低秩与稀疏成分在凸优化眼中的互相独立性。

****：JL 随机投影引理和经典的 Eckart-Young-Mirsky 定理联手为海量维度数据的欧氏距离约减提供了深邃的概率保证。

****：Courant-Fischer 极小极大定理和 Weyl 不等式彻底确立了特征值作为流形上最优化结果的绝对地位，并为整个扰动分析确立了数学疆界。
