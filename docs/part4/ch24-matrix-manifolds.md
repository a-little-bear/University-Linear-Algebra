...
1. **[流形] 为什么一般不能直接使用经典欧氏空间的梯度下降法来优化正交矩阵 $X$（即在约束 $X^TX=I$ 下）？**
   ??? success "参考答案"
       欧氏梯度下降的一步更新 $X - \alpha \nabla f(X)$ 会使得更新后的矩阵不可避免地脱离正交群流形（$X^TX \neq I$），严重破坏物理或几何的正交约束。

2. **[切空间] $n$ 阶正交群 $O(n)$ 在单位阵 $I$ 处的切空间是什么？**
   ??? success "参考答案"
       是所有 $n \times n$ 的反对称矩阵（$A^T = -A$）构成的空间。这反映了无限小旋转的生成元本质。

3. **[黎曼度量] 什么是黎曼度量（Riemannian Metric）在矩阵流形上的几何意义？**
   ??? success "参考答案"
       它是在流形的每个切空间上定义的一个平滑变化的内积，使得我们可以测量流形上曲线的真实长度、切向量的夹角，并据此定义“黎曼梯度”这种内禀的方向导数。

4. **[收回映射] 流形优化算法中的收回映射（Retraction）起到了什么关键作用？**
   ??? success "参考答案"
       它将切空间中的点（如沿着切线方向试探性迈出的一步）以极低的计算代价平滑地“拉回”到原流形面上，同时在极小步长下保持与流形的一阶相切性质，从而实现流形上的合法离散迭代。

5. **[Grassmann] Grassmann 流形 $\operatorname{Gr}(k, n)$ 和 Stiefel 流形 $\operatorname{St}(k, n)$ 有什么根本区别？**
   ??? success "参考答案"
       Stiefel 流形上的点是 $n$ 维空间中的 $k$ 个正交“基向量”的有序组合（带有一个刚性的框架）；而 Grassmann 流形上的点是 $n$ 维空间中的 $k$ 维“子空间本身”，完全不关心子空间内部基的具体选取（商掉了一个 $O(k)$ 的内部旋转自由度）。

6. **[正定流形] 正定矩阵空间 $\mathcal{P}(n)$ 已经是欧氏空间中的凸锥（凸集），为什么还要大费周章地将其视为黎曼流形进行优化？**
   ??? success "参考答案"
       因为欧氏几何的度量并不尊重正定矩阵固有的乘法对称性（如对求逆 $A \mapsto A^{-1}$ 不具备等距性）。基于仿射不变度量的黎曼几何能完美反映这一点，在此度量下 $\mathcal{P}(n)$ 变成了非正曲率流形，避免了欧氏均值导致行列式“异常膨胀”的物理谬误。

7. **[梯度] 设目标函数 $f(X)$ 在欧氏空间下的梯度为 $\nabla f(X)$，如何求它在嵌入子流形 $\mathcal{M}$ 上的黎曼梯度 $\operatorname{grad} f(X)$？**
   ??? success "参考答案"
       只需将欧氏梯度直接正交投影到流形在该点 $X$ 处的切空间上，即 $\operatorname{grad} f(X) = \operatorname{Proj}_{T_X \mathcal{M}}(\nabla f(X))$。

8. **[应用] 主成分分析（PCA）可以等价地看作在哪个流形上的优化问题？**
   ??? success "参考答案"
       Grassmann 流形 $\operatorname{Gr}(k, n)$。因为 PCA 旨在寻找一个使得数据投影方差最大化的整体 $k$ 维子空间，而不在乎这个子空间内部的正交基具体是怎么人为规定的。

9. **[李群] 所有矩阵李群都可以通过指数映射 $\exp(A)$ 完全生成吗？**
   ??? success "参考答案"
       一般只能生成包含单位元的那一个连通分支（例如对于 $O(n)$，指数映射只能生成行列式为 $+1$ 的 $SO(n)$ 分支）。但在单位元的局部邻域上，指数映射建立了李代数到李群的完美微分同胚。

10. **[爱因斯坦思考题] 广义相对论指出真实时空是一个伪黎曼流形。为什么工程优化理论发展到今天，也必须抛弃平坦的欧氏空间，转而在黎曼流形上（如正交群、球面上）设计算法？**
    ??? success "参考答案"
        因为很多物理、工程或深度学习问题（如大模型权重正交化、姿态估计）的解空间天然就是弯曲的且具有内禀对称群（如旋转不变性、能量守恒面）。如果在平坦空间中强行添加罚函数式“硬约束”，会使得优化曲面布满极度病态的断崖和峡谷；而转换视角，将非线性的物理约束直接内化为黎曼流形的本征几何，那么原本充满陷阱的复杂约束优化，就化境为了流形上自然顺滑的“无约束”测地线自由滑行。

## 本章小结

本章将微积分与几何从平直空间跃升到了弯曲的矩阵空间之上，系统介绍了矩阵流形的理论框架和现代计算方法：

1. **基本概念**：嵌入子流形、切空间、Riemannian 度量为含有非线性矩阵约束的问题提供了革命性的几何语言。
2. **一般线性群** $GL(n)$：是所有矩阵 Lie 群的母群，其 Lie 代数 $\mathfrak{gl}(n)$ 即全矩阵空间。
3. **正交群与酉群** $O(n), U(n)$ 及其特殊子群是最基本的紧 Lie 群，Lie 代数分别为反演平移生成的反对称和反 Hermite 矩阵空间。
4. **Stiefel 流形** $\operatorname{St}(k, n)$：参数化标准正交 $k$-组，是正交约束优化的自然空间。
5. **Grassmann 流形** $\operatorname{Gr}(k, n)$：参数化 $k$ 维子空间，商空间的视角提供了子空间间的自然距离（主角）。
6. **正定矩阵流形** $\mathcal{P}(n)$：在仿射不变黎曼度量下是非正曲率空间，具有唯一的 Frechet 几何均值。
7. **矩阵 Lie 群与 Lie 代数**：通过强大的指数映射和 BCH 公式，将高度弯曲的群乘法降维打击为平直空间上的线性代数运算。
8. **流形优化算法**：将约束优化转化为 Riemannian 无约束优化，**Riemannian 梯度**（切空间投影）和**收回映射**（Retraction，流形游走）构成了所有现代流形深度学习框架的核心工具。
