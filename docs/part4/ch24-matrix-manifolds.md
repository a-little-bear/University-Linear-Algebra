...
****：嵌入子流形、切空间、Riemannian 度量为含有非线性矩阵约束的问题提供了革命性的几何语言。

**** $GL(n)$：是所有矩阵 Lie 群的母群，其 Lie 代数 $\mathfrak{gl}(n)$ 即全矩阵空间。

**** $O(n), U(n)$ 及其特殊子群是最基本的紧 Lie 群，Lie 代数分别为反演平移生成的反对称和反 Hermite 矩阵空间。

**** $\operatorname{St}(k, n)$：参数化标准正交 $k$-组，是正交约束优化的自然空间。

**** $\operatorname{Gr}(k, n)$：参数化 $k$ 维子空间，商空间的视角提供了子空间间的自然距离（主角）。

**** $\mathcal{P}(n)$：在仿射不变黎曼度量下是非正曲率空间，具有唯一的 Frechet 几何均值。

****：通过强大的指数映射和 BCH 公式，将高度弯曲的群乘法降维打击为平直空间上的线性代数运算。

****：将约束优化转化为 Riemannian 无约束优化，**Riemannian 梯度**（切空间投影）和**收回映射**（Retraction，流形游走）构成了所有现代流形深度学习框架的核心工具。
