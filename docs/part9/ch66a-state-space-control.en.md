...
1. **[Linearization] Analyze the structure of the general solution to the linear state-space equation $\dot{x} = Ax + Bu$, and prove that it consists of the zero-input response and the zero-state response.**
   ??? success "Solution"
...
2. **[Controllability] Prove: If the eigenvalues $\lambda_i$ of matrix $A$ are distinct, then the system $(A, B)$ is controllable if and only if the projection of $B$ onto each eigenspace of $A$ is non-zero.**
   ??? success "Solution"
       This is a direct consequence of the PBH criterion. If the projection of $B$ onto the left eigenvector $v_i^T$ corresponding to $\lambda_i$ is zero ($v_i^T B = 0$), then $\operatorname{rank}[\lambda_i I - A, B] < n$, rendering that mode uncontrollable.
...
3. **[Observability] Derive the least-squares estimation algorithm for the initial state $x(0)$ of a linear system, and explain why the observability matrix $\mathcal{O}$ being full rank is the necessary and sufficient condition for a unique solution.**
   ??? success "Solution"
       The sequence of observation equations $y^{(k)}(0) = CA^k x(0)$ forms an overdetermined system $Y = \mathcal{O}x(0)$. The solution is $x(0) = (\mathcal{O}^T \mathcal{O})^{-1} \mathcal{O}^T Y$. The existence of the unique inverse requires $\mathcal{O}$ to be of full column rank.

4. **[Lyapunov] Prove: For a stable system, the solution $W_c$ to $A W_c + W_c A^T + BB^T = 0$ characterizes the energy ellipsoid in the state space.**
   ??? success "Solution"
       The controllability Gramian is $W_c = \int_0^\infty e^{At}BB^T e^{A^T t}dt$. Using the solution representation and Cauchy-Schwarz, it can be proven that the minimum control energy required to reach state $x$ is $x^T W_c^{-1} x$.
...
6. **[Kalman Decomposition] Describe the structure of the Kalman canonical decomposition and explain why only the controllable and observable subsystem affects the external transfer function.**
   ??? success "Solution"
       The decomposition partitions the space into four subspaces (能控且能观，能控不能观，等). Uncontrollable or unobservable modes vanish in the computation of $(sI-A)^{-1}$ due to common factor cancellation between the numerator and denominator (pole-zero cancellation).
...
7. **[McMillan Degree] Define the McMillan degree of a transfer function and explain its relationship to the order of the minimal realization.**
   ??? success "Solution"
       The McMillan degree is the total degree of the denominator polynomials in the Smith-McMillan form of the transfer function matrix. It represents the minimum state dimension $n$ required to implement the given input-output mapping.

8. **[Hankel Singular Values] Prove that the eigenvalues of the product of the controllability and observability Gramians are coordinate invariants of the system.**
   ??? success "Solution"
       After transformation, $\bar{W}_c = T^{-1}W_c T^{-T}$ and $\bar{W}_o = T^T W_o T$. Thus $\bar{W}_c \bar{W}_o = T^{-1} W_c W_o T$, meaning the product matrices are similar and share the same eigenvalues (squares of the Hankel singular values).
...
9. **[Balanced Truncation] Detail the criteria for balanced truncation model reduction and its error upper bound in the $H_\infty$ norm.**
   ??? success "Solution"
       In a balanced realization where $W_c=W_o=\Sigma$, states corresponding to small diagonal entries in $\Sigma$ are discarded. The error upper bound is $\Delta \le 2\sum \sigma_{dropped}$. This provides a guarantee for the fidelity of the reduced-order model.

10. **[Stability Assignment] Using algebraic criteria, prove: If $(A, B)$ is controllable, then the poles of the closed-loop system can be assigned to any position in the complex plane via state feedback.**
    ??? success "Solution"
        This is the Pole Placement Theorem. Controllability ensures that the input $u$ can independently affect all invariants of the system matrix $A$, thereby allowing arbitrary modification of the characteristic polynomial via the feedback gain matrix $K$.
