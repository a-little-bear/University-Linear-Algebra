...
1. **[线性化] 分析线性状态空间方程 $\dot{x} = Ax + Bu$ 的通解结构，并证明其由零输入响应与零状态响应组成。**
   ??? success "参考答案"
...
4. **[Lyapunov] 证明：对于稳定系统，$A W_c + W_c A^T + BB^T = 0$ 的解 $W_c$ 刻画了状态空间中的能量椭球。**
   ??? success "参考答案"
       能控性 Gramian $W_c = \int_0^\infty e^{At}BB^T e^{A^T t}dt$。利用解的表示和 Cauchy-Schwarz 不等式可证，移动到状态 $x$ 所需的最小控制能量为 $x^T W_c^{-1} x$。
...
8. **[Hankel奇异值] 证明能控性与能观性 Gramian 乘积的特征值是系统的坐标不变量。**
   ??? success "参考答案"
       变换后 $\bar{W}_c = T^{-1}W_c T^{-T}$ 且 $\bar{W}_o = T^T W_o T$。则 $\bar{W}_c \bar{W}_o = T^{-1} W_c W_o T$，即新旧乘积阵相似，特征值相同（即 Hankel 奇异值的平方）。

9. **[平衡截断] 详述平衡截断的降阶准则及其在 $H_\infty$ 范数下的误差上界。**
   ??? success "参考答案"
       在 $W_c=W_o=\Sigma$ 的平衡实现中保留对应大奇异值的状态。误差上界为 $\Delta \le 2\sum \sigma_{dropped}$。这为大规模动力系统的模型简化提供了先验精度保证。

10. **[稳定性判定] 利用代数判据证明：若 $(A, B)$ 能控，则通过状态反馈 $u = -Kx$ 可以将闭环系统的极点配置在复平面的任何位置。**
    ??? success "参考答案"
        这是极点配置定理。能控性确保了输入 $u$ 能够独立影响系统矩阵 $A$ 的所有不变量，从而允许通过反馈增益阵 $K$ 任意修改特征多项式的系数。
