...
**本章脉络**：齐次坐标 → 仿射变换 → 旋转表示 → 模型-视图-投影矩阵 → 透视与正交投影 → 视口变换 → 颜色空间变换 → 样条曲面
...
2. **[齐次除法] 推导透视投影矩阵中 $w$ 分量的生成逻辑及其在齐次除法中的作用。**
   ??? success "参考答案"
       透视投影矩阵将 $z$ 信息编码到 $w$ 分量（通常 $w = -z$）。在裁剪之后，所有坐标除以 $w$，实现了 $x/z, y/z$ 的映射，这在几何上等价于相似三角形比例关系，产生了视觉上的透视效应。

3. **[旋转奇异性] 分析 Euler 角表示法在 $\theta = \pm 90^\circ$ 时出现“万向锁”的数学本质。**
   ??? success "参考答案"
...
5. **[插值特性] 证明 SLERP 插值公式在单位球面上具有恒定的角速度。**
   ??? success "参考答案"
       SLERP 插值通过 $\frac{\sin((1-t)\theta)}{\sin\theta} q_0 + \frac{\sin(t\theta)}{\sin\theta} q_1$ 进行。通过计算其相对于 $t$ 的一阶导数模长，可以证明其在 $S^3$ 上的弧长演化速率恒定，即 $\frac{d\alpha}{dt} = \text{const}$。

6. **[法向量校正] 计算非均匀缩放矩阵 $S = \operatorname{diag}(2, 1, 1)$ 对表面法向量 $(1, -1, 0)^T$ 的影响。**
   ??? success "参考答案"
       顶点矩阵 $M=S$。正确法向变换矩阵为 $(M^{-1})^T = \operatorname{diag}(0.5, 1, 1)$。变换后法向量为 $(0.5, -1, 0)^T$。变换后切向量（初始为 $(1, 1, 0)$）为 $(2, 1, 0)$。其内积为 $0.5(2) + (-1)(1) = 0$，证实法线仍保持正交。

7. **[深度非线性] 分析透视变换后深度值 $z_{ndc}$ 与原始深度 $z_{eye}$ 的倒数关系。**
   ??? success "参考答案"
       $z_{ndc} = A + B/z_{eye}$。这种双曲关系导致近平面附近的采样密度极大，而远平面附近采样极稀疏。这有利于近处物体的精确遮挡判定，但也导致了远景处的深度冲突 (Z-fighting)。

8. **[重心坐标] 证明透视校正插值公式 $a = \frac{\sum a_i \lambda_i / w_i}{\sum \lambda_i / w_i}$ 能够还原属性的线性。**
   ??? success "参考答案"
       这源于从三维世界空间到屏幕空间的射影变换不是仿射的。只有通过 $1/w$ 加权，才能将非线性的屏幕坐标系重新映射回线性变化的剪裁空间参数。

9. **[反射与对合] 证明 Householder 反射矩阵 $H = I - 2\mathbf{n}\mathbf{n}^T$ 是对称且正交的。**
   ??? success "参考答案"
       $H^T = H$ 显见。$H^T H = (I-2\mathbf{n}\mathbf{n}^T)(I-2\mathbf{n}\mathbf{n}^T) = I - 4\mathbf{n}\mathbf{n}^T + 4\mathbf{n}(\mathbf{n}^T\mathbf{n})\mathbf{n}^T = I$（由于 $\|\mathbf{n}\|=1$）。其行列式始终为 $-1$。

10. **[颜色变换] 说明 RGB 空间到 YCbCr 空间线性变换矩阵中，亮度分量 $Y$ 系数的确定依据。**
    ??? success "参考答案"
