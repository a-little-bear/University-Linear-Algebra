...
2. **[D-H参数] 分析 Denavit-Hartenberg 参数化方法中各分量矩阵的代数结构。**
   ??? success "参考答案"
       D-H 变换由绕 $z$ 轴旋转/平移和绕 $x$ 轴旋转/平移复合而成。它要求相邻 $z$ 轴的公垂线作为 $x$ 轴，从而将一般 6 参数变换缩减为 4 参数，实现了运动学建模的标准化。

3. **[逆运动学] 说明逆运动学（IK）解的不唯一性及其在代数上对应的多解现象。**
   ??? success "参考答案"
       IK 求解等价于求解矩阵方程 $T(\theta) = T_{target}$。由于涉及 $\sin, \cos$ 项的周期性，该非线性方程组在定义域内通常对应多个孤立解（如“肘部向上”与“肘部向下”构型），或因工作空间限制而无解。

4. **[Jacobian推导] 证明：对于 $n$ 轴机械臂，其雅可比矩阵的第 $i$ 列对应于第 $i$ 个关节在末端产生的速度分量。**
   ??? success "参考答案"
       由全微分关系 $v = \sum \frac{\partial T_e}{\partial \theta_i} \dot{\theta_i}$。对于转动关节，第 $i$ 列反映了该特定关节的转动对末端整体线速度和角速度的几何贡献。

5. **[奇异性分析] 分析雅可比矩阵 $\det(J J^T) = 0$ 时的代数意义及其对控制系统的影响。**
   ??? success "参考答案"
       奇异点意味着 $J$ 亏秩，系统在任务空间失去一个或多个自由度。此时逆运动学速度解 $\dot{\theta} = J^{-1} v$ 会产生无穷大关节速度，导致执行器发生物理损坏。
...
6. **[计算] 给定 2 自由度平面连杆（长度 $l_1, l_2$），推导其雅可比矩阵并找出所有的奇异构型。**
   ??? success "参考答案"
       ...
       其行列式为 $\det(J) = l_1 l_2 s_2$。奇异点发生在 $\theta_2 = 0$ 或 $\pi$ 时（即连杆完全伸直或折叠）。

7. **[可操作性] 定义 Yoshikawa 可操作性指标 $w = \sqrt{\det(J J^T)}$ 并说明其作用。**
   ??? success "参考答案"
       该指标衡量了末端在各方向运动能力的“体积”。在轨迹规划中，应尽量避开 $w \to 0$ 的区域，以保证机器人有足够的运动余量来补偿扰动。

8. **[虚功原理] 利用线性代数证明关节力矩 $\tau$ 与末端作用力 $f$ 满足平衡方程 $\tau = J^T f$。**
   ??? success "参考答案"
       由能量守恒定律：$f^T v = \tau^T \dot{\theta}$。代入 $v = J \dot{\theta}$ 得 $f^T J \dot{\theta} = \tau^T \dot{\theta}$。由于 $\dot{\theta}$ 任意，故 $\tau^T = f^T J \implies \tau = J^T f$。这定义了力空间与运动空间之间的对偶映射。

9. **[冗余解析] 对于冗余机器人（$n > 6$），解释如何利用 $J$ 的广义逆和零空间投影。**
   ??? success "参考答案"
       通解为 $\dot{\theta} = J^\dagger v + (I - J^\dagger J) z$。其中 $z$ 是任意向量，项 $(I - J^\dagger J) z$ 属于 $J$ 的零空间。这种“内部运动”不改变末端位姿，可用于避障。

10. **[螺旋理论] 简述指数坐标表示法 (Product of Exponentials) 相比传统 D-H 方法的优势。**
    ??? success "参考答案"
        PoE 公式基于 Lie 代数的指数映射 $e^{\hat{\xi}\theta}$。它将整个运动链视为一系列螺旋运动的复合，避免了局部坐标系定义的歧义，在微分分析中具有更好的全局一致性。
...
2. **微分运动学**：引入雅可比矩阵作为系统的核心算子，揭示了空间速度与关节力矩的转换关系。
3. **性能评价**：确立了奇异性与可操作性等代数指标，为机器人构型优化提供了定量判据。
