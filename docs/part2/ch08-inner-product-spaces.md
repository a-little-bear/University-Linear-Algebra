...
1. **[概念] 所有的范数都由某个内积诱导吗？如何判断？**
   ??? success "参考答案"
       否。满足平行四边形恒等式（$\|\mathbf{u}+\mathbf{v}\|^2 + \|\mathbf{u}-\mathbf{v}\|^2 = 2\|\mathbf{u}\|^2 + 2\|\mathbf{v}\|^2$）的范数才是由内积诱导的（Jordan-von Neumann 定理）。

2. **[不等式] 柯西-施瓦茨不等式（Cauchy-Schwarz）等号成立的几何条件是什么？**
   ??? success "参考答案"
       等号成立当且仅当两向量线性相关，即共线（同向或反向）。

3. **[伴随] 如果线性算子 $A$ 在标准正交基下的矩阵为 $M$，其伴随算子 $A^*$ 的矩阵是什么？**
   ??? success "参考答案"
       是 $M$ 的共轭转置矩阵 $M^H$（实数域上退化为 $M^T$）。

4. **[正交投影] 为什么正交投影矩阵 $P$ 必定是自伴（Hermitian）的？**
   ??? success "参考答案"
       因为正交投影确保残差 $\mathbf{v} - P\mathbf{v}$ 垂直于像空间 $\operatorname{Im}(P)$，这等价于对任意 $\mathbf{x}, \mathbf{y}$，$\langle P\mathbf{x}, \mathbf{y}-P\mathbf{y}\rangle = 0$。展开得 $\langle P\mathbf{x}, \mathbf{y}\rangle = \langle P\mathbf{x}, P\mathbf{y}\rangle$。同理交换得 $\langle \mathbf{x}, P\mathbf{y}\rangle = \langle P\mathbf{x}, P\mathbf{y}\rangle$，故 $\langle P\mathbf{x}, \mathbf{y}\rangle = \langle \mathbf{x}, P\mathbf{y}\rangle$，证毕。

5. **[正规算子] 举一个正规矩阵但不自伴的例子。**
   ??? success "参考答案"
       $90^\circ$ 旋转矩阵 $\begin{pmatrix} 0 & -1 \\ 1 & 0 \end{pmatrix}$。它满足 $A^T A = A A^T = I$，但显然不对称。

6. **[酉算子] 酉矩阵（或正交矩阵）的特征值有什么特殊性质？**
   ??? success "参考答案"
       它们所有的特征值模长都为 1（在复平面上分布于单位圆周上），体现了保距变换的旋转性质。

7. **[谱定理] 实对称矩阵的特征向量一定能正交归一化吗？**
   ??? success "参考答案"
       一定能。因为不同特征值对应的特征向量天然正交；而对于相同特征值（重根）的特征子空间，可以通过 Gram-Schmidt 正交化构造出一组正交基。

8. **[距离] 欧几里得空间中，一点到某子空间的距离如何求？**
   ??? success "参考答案"
       等于该点的向量减去它在该子空间上的正交投影向量，所得的“残差向量”的范数（长度）。这是最佳逼近定理的核心。

9. **[无穷维] 函数空间 $L^2$ 上的积分内积 $\langle f, g \rangle = \int f(x)g(x)dx$ 为什么重要？**
   ??? success "参考答案"
       它是傅里叶级数以及所有正交多项式（如勒让德、切比雪夫）的几何基础，将连续函数巧妙地视为拥有“长度”和“角度”的无限维向量。

10. **[爱因斯坦思考题] 量子力学中，物理可观测量必须对应什么算子？这在线性代数中是如何被保证的？**
    ??? success "参考答案"
        必须对应**自伴（厄米特）算子**。因为在内积空间（希尔伯特空间）中，只有自伴算子能保证其特征值必然是**实数**。我们用仪器测量出的任何物理量（如能量、动量）只能是实数值，这与数学公理形成了不可思议的统一。

## 本章小结

本章系统建立了内积空间的理论框架，主要内容包括：

1. **内积与范数**：公理化定义了内积，诱导出范数和距离，并证明了极其重要的 Cauchy-Schwarz 不等式和平行四边形恒等式。
2. **正交性**：引出了正交补和正交直和分解，证明了正交投影是最优逼近。
3. **伴随算子**：将内积作用在算子两端引发了伴随概念 $T^*$，是研究算子对称性的基础。
4. **正规与自伴**：$TT^* = T^*T$ 刻画了算子的“结构完备性”，自伴算子确保了特征值全为实数。
5. **谱定理**：全章的顶点，证明了正规算子和自伴算子可以通过选取标准正交基完全对角化，揭示了对称阵和正交阵的深刻几何内涵。
