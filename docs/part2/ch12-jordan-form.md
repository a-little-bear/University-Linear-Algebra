...
??? proof "证明"
    ...
    **唯一性：**
    ...
    （约定 $r_0 = n$），这由 $A$ 唯一确定。$\blacksquare$
...
1. **[对角化] 矩阵能够完全对角化的充要条件是什么？**
   ??? success "参考答案"
       矩阵的每一个特征值的代数重数必须等于其几何重数（即所有特征空间维数之和等于全空间维数，或者所有 Jordan 块全为 $1 \times 1$）。

2. **[Jordan 块] $k \times k$ 的 Jordan 块 $J_k(\lambda)$ 有多少个线性无关的特征向量？**
   ??? success "参考答案"
       仅有 1 个。它位于该块的“顶部”（对应标准基的第一个分量或相关方向）。因此它的几何重数为 1。

3. **[幂零阵] 若 $N$ 是 $n \times n$ 的幂零矩阵（即存在 $k$ 使 $N^k = 0$），那么它的所有特征值必须是什么？**
   ??? success "参考答案"
       全都是 0。这是因为若 $N\mathbf{x} = \lambda \mathbf{x}$，则 $N^k\mathbf{x} = \lambda^k \mathbf{x} = 0$，故 $\lambda = 0$。

4. **[广义空间] 如果特征值 $\lambda$ 的代数重数为 $m$，其广义特征空间 $G_\lambda = \ker((A-\lambda I)^n)$ 的实际维数是多少？**
   ??? success "参考答案"
       正好等于 $m$。广义特征向量恰好“补足”了由几何重数缺失而不足的基向量，恢复了直和分解。

5. **[最小多项式] 如果一个矩阵的最小多项式没有重根（形如 $\prod (\lambda - \lambda_i)$），它具备什么极好的代数性质？**
   ??? success "参考答案"
       它必然可以完全对角化。因为此时每个特征值对应的最大 Jordan 块大小都是 1。

6. **[Cayley-Hamilton] $A$ 的特征多项式 $p(\lambda)$ 决定了 $p(A) = 0$。最小多项式 $m(\lambda)$ 和 $p(\lambda)$ 之间有何整除关系？**
   ??? success "参考答案"
       最小多项式 $m(\lambda)$ 一定整除特征多项式 $p(\lambda)$，并且它们拥有完全相同的根集，唯一的区别是根的重数可能较小。

7. **[矩阵函数] 在利用 Jordan 块计算矩阵指数 $e^{J_k(\lambda)t}$ 时，它的非对角线上会产生什么类型的函数？**
   ??? success "参考答案"
       会产生多项式项（如 $t, t^2/2!, \dots$）与指数 $e^{\lambda t}$ 的乘积。这就是为什么在求解常微分方程组时，如果特征方程出现重根，解系中会出现 $t e^{\lambda t}$ 的原因。

8. **[相似判定] 两个方阵具有完全相同的特征多项式和最小多项式，它们就一定相似吗？**
   ??? success "参考答案"
       不一定（除非 $n \le 3$）。例如两个 $4 \times 4$ 矩阵，一个包含两个 $J_2(0)$，另一个包含一个 $J_2(0)$ 和两个 $J_1(0)$。它们的最小多项式都是 $\lambda^2$，特征多项式都是 $\lambda^4$，但它们拥有不同的 Jordan 标准形，因而不相似。

9. **[迹与行列式] 如何通过 Jordan 标准形快速证明 $\operatorname{tr}(A) = \sum \lambda_i$ 和 $\det(A) = \prod \lambda_i$？**
   ??? success "参考答案"
       由于 $A$ 相似于 Jordan 标准形 $J$，且相似变换不改变迹和行列式。而在 $J$ 中，主对角线上的元素恰好列出了所有的特征值（计代数重数），所以等式显然成立。

10. **[爱因斯坦思考题] 为什么大自然在绝大多数随机给定的系统中会呈现“可对角化”的状态，而纯粹的“不可对角化（含大 Jordan 块）”系统通常极其脆弱且罕见（例如临界阻尼）？**
    ??? success "参考答案"
        不可对角化意味着特征空间的“坍缩”和退化。在参数空间中，这种状态属于“测度为零”的临界点（如共振频率完全精确重合）。一旦施加任何微小的非对称随机扰动（大自然充满噪音），重根就会立刻散开，矩阵结构将重新恢复为可对角化。Jordan 形的出现标志着系统处于极度特殊的对称或受迫相变边缘。
