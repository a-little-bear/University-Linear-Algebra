...
1. **[Cayley-Hamilton] 利用 Cayley-Hamilton 定理证明，可逆矩阵 $A$ 的逆矩阵 $A^{-1}$ 可以写成 $A$ 的多项式。**
   ??? success "参考答案"
       设特征多项式 $p(\lambda) = \lambda^n + c_{n-1}\lambda^{n-1} + \dots + c_0 = 0$。由 $p(A)=0$ 可知 $A^n + c_{n-1}A^{n-1} + \dots + c_0 I = 0$。因为 $A$ 可逆，故 $c_0 = (-1)^n \det(A) \neq 0$。移项并同乘 $c_0^{-1} A^{-1}$，即可用 $A$ 的多项式表示 $A^{-1}$。

2. **[谱半径] 矩阵幂级数 $\sum c_k A^k$ 收敛的充要条件与矩阵的什么性质有关？**
   ??? success "参考答案"
       谱半径 $\rho(A)$ 必须严格小于标量幂级数 $\sum c_k z^k$ 的收敛半径 $R$。

3. **[矩阵指数] 计算 $\frac{d}{dt} e^{At}$。**
   ??? success "参考答案"
       利用幂级数逐项求导，容易证明其等于 $A e^{At}$ 或 $e^{At} A$。

4. **[性质] 为什么 $e^{A+B} = e^A e^B$ 一般不成立？在什么条件下成立？**
   ??? success "参考答案"
       因为矩阵乘法不满足交换律，交叉项无法像标量那样合并。当 $A$ 和 $B$ 交换（$AB=BA$）时该式成立。

5. **[行列式] 证明雅可比恒等式 $\det(e^A) = e^{\operatorname{tr}(A)}$。**
   ??? success "参考答案"
       通过 Schur 三角化或 Jordan 标准形，主对角线上的元素即为特征值。矩阵指数的特征值为 $e^{\lambda_i}$。其乘积即为 $e^{\sum \lambda_i} = e^{\operatorname{tr}(A)}$。

6. **[Jordan 形] 为什么不直接在原矩阵上用泰勒展开计算 $e^A$，而要先转化为 Jordan 标准形（或对角化）？**
   ??? success "参考答案"
       高次矩阵乘积计算极其昂贵且易累积舍入误差。对角化将全局耦合的矩阵乘法化为了各维度的独立标量级数运算，大大降低了复杂度。

7. **[矩阵对数] 什么样的实矩阵存在实矩阵对数？**
   ??? success "参考答案"
       这是一个深刻的结论：所有实特征值必须是严格正数，或者所有负特征值必须“成对出现”且具有完全相同的 Jordan 块结构。

8. **[矩阵对数] 对于不可逆矩阵（如幂零阵），为什么不存在矩阵对数？**
   ??? success "参考答案"
       因为如果存在矩阵对数 $X$ 使得 $e^X = A$，由 $\det(A) = e^{\operatorname{tr}(X)}$ 且指数函数在实/复数域上永不为 0 可知，$A$ 的行列式必不为 0，这与不可逆矛盾。

9. **[一般函数] 如果 $f(A)$ 只需要定义在矩阵的特征值上，为什么在遇到代数重数为 $m$ 的特征值时，还需要 $f$ 具备直到 $m-1$ 阶的导数？**
   ??? success "参考答案"
       因为重根可能导致 Jordan 块的出现。在 Jordan 块上应用多项式或级数时，其超对角线项自然会浮现出原函数的各阶导数（泰勒展开的截断）。

10. **[爱因斯坦思考题] 经典物理（如牛顿力学）使用向量和实函数，而量子力学必须使用“矩阵函数”（如时间演化算符 $U(t) = e^{-iHt/\hbar}$）。为什么大自然需要升级到矩阵代数？**
    ??? success "参考答案"
        因为量子世界的核心是不确定性与非对易性。标量函数永远是可交换的，只有算符（矩阵）才能表现出 $[x, p] = i\hbar$ 这种对易子不为零的现象。矩阵函数完美地包容了经典标量函数的平滑演化，同时内嵌了波函数的非局域干涉效应。
