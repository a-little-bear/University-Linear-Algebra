...
## 10.3 Cholesky 分解与 LDL^T 分解

<div class="context-flow" markdown>

Ch9 正定 $A = C^TC$ → 取 $C = L^T$ 得 $A = LL^T$ → **LDLt**：不带平方根的消元形式，适用于一般对称矩阵
...
??? proof "证明"
    **存在性：** 由 LU 分解（正定矩阵的顺序主子式全为正，保证 LU 分解存在）：$A = L_0 U_0$。令 $D = \operatorname{diag}(u_{11}, \ldots, u_{nn})$（$U_0$ 的对角线），则 $U_0 = DU_1$，其中 $U_1$ 是单位上三角。
    ...
    由 $A = A^T$，$L_0DU_1 = U_1^TDL_0^T$。由唯一性可推出 $U_1 = L_0^T$，故 $A = L_0DL_0^T$。
...
!!! definition "定义 10.3a (LDL^T 分解)"
    设 $A$ 是 $n \times n$ 实对称矩阵。若 $A$ 的所有顺序主子式非零，则存在唯一的单位下三角矩阵 $L$ 和对角矩阵 $D$ 使得
    
    $$A = LDL^T$$
    
    这称为 $A$ 的 **LDLt 分解**。与 Cholesky 不同，它不需要计算平方根，且适用于非正定的对称矩阵。
...
1. **[LU] LU 分解存在且唯一的充要条件是什么？**
   ??? success "参考答案"
       矩阵的所有前 $k$ 阶顺序主子式全都不为零，这就保证了高斯消元过程中不需要行交换。

2. **[Cholesky] Cholesky 分解 $A = LL^T$ 只适用于哪类矩阵？**
   ??? success "参考答案"
       实对称正定矩阵（或复厄米特正定矩阵）。它是计算此类矩阵方程组和求行列式最稳定、最快的方法之一。

3. **[应用] 为什么在解方程组 $A\mathbf{x} = \mathbf{b}$ 时，倾向于计算 LU 分解而不是求逆 $A^{-1}$？**
   ??? success "参考答案"
       求逆算法不仅计算量比 LU 多，而且在浮点数运算下更容易累积舍入误差，导致数值不稳定。得到 LU 分解后，解任意新的 $\mathbf{b}$ 只需要两次 $O(n^2)$ 的三角代入即可。

4. **[QR] QR 分解中的 $Q$ 是什么性质的矩阵？$R$ 呢？**
   ??? success "参考答案"
       $Q$ 是正交矩阵（若为缩减分解则是列正交），$R$ 是上三角矩阵。

5. **[QR] 从几何上看，QR 分解等价于什么经典算法？**
   ??? success "参考答案"
       Gram-Schmidt 正交化过程。

6. **[Schur] Schur 分解 $A = UTU^H$ 能否证明所有方阵都可以对角化？**
   ??? success "参考答案"
       不能。$T$ 是上三角矩阵，只有当 $A$ 是正规矩阵时，$T$ 才会退化为对角矩阵。

7. **[谱分解] 对称矩阵的谱分解 $A = \sum \lambda_i \mathbf{q}_i\mathbf{q}_i^T$ 中，$\mathbf{q}_i\mathbf{q}_i^T$ 在几何上代表什么？**
   ??? success "参考答案"
       代表向第 $i$ 个特征向量方向的**正交投影矩阵**，且秩为 1。谱分解将矩阵视为各独立特征频率投影的线性叠加。

8. **[极分解] 极分解 $A = UP$ 中，$U$ 和 $P$ 分别对应复数极坐标 $z = re^{i\theta}$ 中的什么成分？**
   ??? success "参考答案"
       $U$ 是酉矩阵（旋转），对应 $e^{i\theta}$；$P$ 是半正定厄米特阵（伸缩），对应模长 $r$。

9. **[极分解] 如何通过 SVD ($A = V\Sigma W^H$) 快速写出 $A$ 的极分解？**
   ??? success "参考答案"
       $U = VW^H$，$P = W\Sigma W^H$。验证 $UP = V W^H W \Sigma W^H = V \Sigma W^H = A$。

10. **[爱因斯坦思考题] 爱因斯坦的“分而治之”哲学：分解大抵是复杂的系统如何通过有限的简单基石构建。为什么矩阵分解在量子力学（如态的谱分解）和现代数据科学（SVD 降维）中都具有统一的地位？**
    ??? success "参考答案"
        因为矩阵分解揭示了复杂变换的**“本征模式”（eigen-modes 或 singular-modes）**。无论宇宙演化的波函数，还是庞大数据集的隐藏特征，它们在代数底层都可以被投影到一系列相互独立（正交）的基础向量上。这是对复杂现象的最简本质提取。

## 本章小结

本章将求解方程组和特征值问题的算法过程上升为了统一的**矩阵分解**理论，主要内容包括：

1. **LU 分解**：将高斯消元操作系统化地写成矩阵形式。通过 PLU 分解，任何方阵都可被消元。
2. **Cholesky 与 LDLt 分解**：专门针对正定对称矩阵和一般对称矩阵的高效消元算法。
3. **QR 分解**：将任意矩阵分解为正交阵和上三角阵，反映了对原列向量实施的几何正交化操作。它是求解最小二乘的利器。
4. **Schur 三角化**：从理论层面证明了复空间上方阵必定可酉三角化，这是很多特征值算法的基础。
5. **谱分解与极分解**：进一步深化了对正规阵和一般阵内禀结构的认识，其中极分解将任意映射彻底解耦为旋转和放缩。
