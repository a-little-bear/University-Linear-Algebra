...
1. **[奇异值] 奇异值 $\sigma_i$ 与矩阵 $A^TA$ 的特征值 $\lambda_i$ 有何关系？**
   ??? success "参考答案"
       $\sigma_i = \sqrt{\lambda_i(A^TA)}$。奇异值一定是非负实数。

2. **[范数] 矩阵的谱范数 $\|A\|_2$ 与其奇异值有什么关系？**
   ??? success "参考答案"
       $\|A\|_2 = \sigma_1(A)$，即最大的奇异值。

3. **[满秩] 假设 $A$ 是 $m \times n$ 的满秩矩阵，它的非零奇异值有多少个？**
   ??? success "参考答案"
       有 $\min(m, n)$ 个非零奇异值（即秩的数量）。

4. **[几何] 在二维平面，一个非奇异矩阵通过 SVD 作用在单位圆上，会将其变成什么形状？**
   ??? success "参考答案"
       会变成一个椭圆。半长轴和半短轴的长度就是该矩阵的两个奇异值 $\sigma_1, \sigma_2$。方向由对应的左奇异向量 $\mathbf{u}_1, \mathbf{u}_2$ 给出。

5. **[低秩逼近] 定理 Eckart-Young-Mirsky 告诉我们，如何找到矩阵 $A$ 的最佳秩 $k$ 近似？**
   ??? success "参考答案"
       对 $A$ 进行 SVD 分解后，只保留前 $k$ 大的奇异值及其对应的左右奇异向量，将其余部分（或者奇异值置零）得到的截断矩阵 $A_k$ 即可。

6. **[条件数] 对于可逆方阵 $A$，使用奇异值如何计算其 2-范数条件数 $\kappa_2(A)$？**
   ??? success "参考答案"
       $\kappa_2(A) = \sigma_{\max} / \sigma_{\min} = \sigma_1 / \sigma_n$。

7. **[伪逆] 利用 SVD ($A=U\Sigma V^T$)，如何显式构造 Moore-Penrose 伪逆 $A^\dagger$？**
   ??? success "参考答案"
       $A^\dagger = V\Sigma^\dagger U^T$，其中 $\Sigma^\dagger$ 是将 $\Sigma$ 的非零奇异值取倒数并转置得到的矩阵。

8. **[对称阵] 对于实对称矩阵，它的 SVD 和谱分解（特征值分解）有什么区别？**
   ??? success "参考答案"
       谱分解中的特征值可以为负，而 SVD 的奇异值全部非负。若特征值 $\lambda < 0$，奇异值取其绝对值 $\sigma = |\lambda|$，同时吸收负号到对应的（左或右）奇异向量中。

9. **[数据压缩] 为什么 SVD 能用于图像压缩？**
   ??? success "参考答案"
       因为图像可以看作矩阵，而自然图像的奇异值通常呈陡峭的长尾衰减。保留前几个最大奇异值（低秩近似）就能抓住图像的绝大部分信息，大幅减少所需存储的参数量。

10. **[爱因斯坦思考题] SVD 揭示了任意一个复杂的线性映射都可以分解为：旋转 $\to$ 各轴独立拉伸 $\to$ 再次旋转。如果将“拉伸”视作不同维度的重要性权重，这在物理世界中意味着什么？**
    ??? success "参考答案"
        它意味着宇宙中任何复杂的线性相互作用，在其“固有主轴”（右奇异向量空间）上都是完全解耦和独立的。只要找准了观测框架（左奇异向量空间），哪怕错综复杂的网络也只是几个互不干扰的核心独立机制的叠加。
