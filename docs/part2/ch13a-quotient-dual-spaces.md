...
!!! example "例 13A.13"
    ...
    这展示了幂等分解前在对偶空间中的完美对称性。

## 练习题

1. **[概念] 为什么商空间 $V/W$ 可以被直观地视为将子空间 $W$ “坍缩”为一个点（零向量）？**
   ??? success "参考答案"
       在商空间中，$W$ 本身就是加法单位元（零向量）。任何属于 $W$ 的向量 $w$ 满足 $w + W = 0 + W = W$。这在代数上实现了对 $W$ 内部细节的忽略，只保留 $V$ 中与 $W$ 垂直（或互补）方向的信息。

2. **[维数] 设 $V$ 是 10 维空间，$W$ 是 3 维子空间。求 $\dim(V/W)$ 和 $\dim W^0$。**
   ??? success "参考答案"
       $\dim(V/W) = \dim V - \dim W = 10 - 3 = 7$。
       $\dim W^0 = \dim V - \dim W = 10 - 3 = 7$。
       注意这两个空间的维数相等并非巧合，它们之间存在自然同构。

3. **[对偶基] 设 $V = \mathbb{R}^2$，基为 $\{e_1=(1,0), e_2=(1,1)\}$。求对偶基 $\{e_1^*, e_2^*\}$ 在标准基下的表达式。**
   ??? success "参考答案"
       设 $e_1^*(x,y) = ax+by$，$e_2^*(x,y) = cx+dy$。
       由 $e_1^*(e_1)=1, e_1^*(e_2)=0 \Rightarrow a=1, a+b=0 \Rightarrow a=1, b=-1$。故 $e_1^*(x,y) = x - y$。
       由 $e_2^*(e_1)=0, e_2^*(e_2)=1 \Rightarrow c=0, c+d=1 \Rightarrow c=0, d=1$。故 $e_2^*(x,y) = y$。

4. **[零化子] 证明：若 $U \subseteq W$，则 $W^0 \subseteq U^0$。这种“反包含”关系说明了什么？**
   ??? success "参考答案"
       若泛函 $\varphi$ 在更大的集合 $W$ 上全为 0，它显然在 $W$ 的子集 $U$ 上也全为 0。这种反包含关系体现了对偶性（Duality）中的逆变性质：空间越大，能将其全部“归零”的观测器（泛函）就越少。

5. **[转置] 证明矩阵的迹函数 $\operatorname{tr}: \mathbb{R}^{n \times n} \to \mathbb{R}$ 是一个线性泛函。它的核是什么？**
   ??? success "参考答案"
       $\operatorname{tr}(A+B) = \operatorname{tr}(A)+\operatorname{tr}(B)$ 且 $\operatorname{tr}(cA) = c\operatorname{tr}(A)$。其核是所有对角线元素之和为 0 的矩阵空间，其维数为 $n^2 - 1$。

6. **[同构] 为什么在有限维情况下 $V \cong V^*$，但我们却说它们之间没有“自然”的同构？**
   ??? success "参考答案"
       虽然它们维数相同，但建立 $V \to V^*$ 的任何同构都必须先选定一组特定的基（或者引入一个内积）。不同的基会给出不同的同构。只有双对偶 $V \to V^{**}$ 的求值映射是不依赖于任何人为选取的基的，因而是“自然”的。

7. **[秩] 利用转置映射证明 $\operatorname{rank}(T) = \operatorname{rank}(T^*)$。**
   ??? success "参考答案"
       由定理 13A.15，$\operatorname{im} T^* \cong (V/\ker T)^* \cong (\operatorname{im} T)^*$。故维数相等。从矩阵角度看，这等价于证明行秩等于列秩。

8. **[不变子空间] 若 $W$ 是 $T: V \to V$ 的不变子空间，证明 $W^0$ 是转置算子 $T^*: V^* \to V^*$ 的不变子空间。**
   ??? success "参考答案"
       设 $\varphi \in W^0$，则对任意 $w \in W$，有 $(T^*\varphi)(w) = \varphi(T(w))$。由于 $W$ 是 $T$-不变的，$T(w) \in W$，故 $\varphi(T(w)) = 0$。因此 $T^*\varphi \in W^0$。

9. **[自然同构] 解释为什么一个向量 $v$ 可以被看作是作用在“观测器”（泛函）上的算子。**
   ??? success "参考答案"
       这就是双对偶的视角：如果我们把泛函 $\varphi$ 看作输入，向量 $v$ 就可以被视为一个函数 $\hat{v}$，它计算 $\hat{v}(\varphi) = \varphi(v)$。在爱因斯坦看来，物体（向量）和测量它的仪器（泛函）是对等的、互为背景的。

10. **[爱因斯坦思考题] 在广义相对论中，切向量（Velocity）位于 $V$，而梯度向量（Gradient of scalar field）位于 $V^*$。为什么我们不能直接把速度和梯度相加？这在代数结构上说明了什么？**
    ??? success "参考答案"
        因为它们属于不同的空间，物理量纲和坐标变换律完全不同（逆变 vs 协变）。只有通过引入“度规张量”（内积），我们才能建立 $V$ 与 $V^*$ 之间的映射。这种“相加”的尝试在数学上是违背范畴逻辑的，反映了宇宙中“运动”与“变化率”之间内禀的对偶对称性。

## 本章小结

本章深入探讨了向量空间的两个抽象构造，将线性代数推向了更高的理论维度：

1. **商空间 $V/W$**：通过陪集构造，实现了对冗余信息的代数简化。同构定理揭示了线性映射的核与像之间的深层联系：像空间本质上就是定义域“模掉”核之后的结构。
2. **对偶空间 $V^*$**：将线性泛函组织成空间，建立了从“观测”角度审视向量空间的范式。对偶基提供了提取坐标的标准化映射。
3. **零化子与转置映射**：零化子刻画了空间及其对偶之间的几何约束关系；转置映射展示了线性变换如何在观测者空间中“反向传播”，并自然地引出了矩阵转置的代数起源。
4. **自然同构与双对偶**：证明了有限维空间与其双对偶空间之间存在不依赖基的“天生”同构，这标志着从具体计算向范畴化思维的飞跃。
5. **对偶性的应用**：统一了解空间、秩理论以及多项式插值的底层逻辑，为研究微分几何中的张量场和物理中的协变/逆变矢量奠定了坚实基础。
