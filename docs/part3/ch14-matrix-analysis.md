...
**前置**：特征值(Ch6) · 范数(Ch15预备)

**脉络**：矩阵空间拓扑 → **谱半径**统治收敛 → **Gershgorin**定位特征值 → 矩阵微积分 → 特征值-奇异值桥梁

**延伸**：谱半径在动力系统稳定性分析、迭代法收敛性判定（Jacobi、Gauss-Seidel）、种群动力学中至关重要；Gershgorin 定理在工程中用于快速估计特征值范围而无需精确计算
...
**脉络**：有限维 $\Rightarrow$ 范数等价 $\Rightarrow$ 逐元素收敛 = 范数收敛 = 完备(Banach)空间
...
**脉络**：绝对收敛(完备性) → **Neumann 级数** $\sum A^k = (I-A)^{-1}$ ←→ 几何级数的矩阵推广 · 收敛条件: $\rho(A)<1$
...
**脉络**：$\rho(A) = \max|\lambda_i|$ → **Gelfand 公式** $\rho(A)=\lim\|A^k\|^{1/k}$ 打通范数与谱 · $\rho(A)\leq\|A\|$ 对任意次可乘范数
...
**洞察**：不求解特征多项式，仅看矩阵元素即可圈定特征值位置 · 圆心 = 对角元，半径 = 非对角行和 → 对角占优 $\Rightarrow$ 可逆
...
**脉络**：逐元素求导 → 乘积法则(注意顺序) → $\frac{d}{dt}A^{-1}=-A^{-1}\dot{A}A^{-1}$ · 关键应用：$\frac{d}{dt}e^{tA}=Ae^{tA}$ → Ch20 ODE基解矩阵
...
**洞察**：$\prod_{i=1}^k|\lambda_i|\leq\prod_{i=1}^k\sigma_i$，取 $k=n$ 等号成立($=|\det A|$) · 奇异值Weyl不等式 → Ch15扰动理论 · Ch18不等式网络
...
**核心结论**：$A^k\to O \Leftrightarrow \rho(A)<1$ · 速率 $\|A^k\|\leq Cr^k$($r>\rho(A)$) → Ch17 Markov链收敛 · Jacobi/Gauss-Seidel迭代收敛判据
...
1. **[微积分] 设 $A(t)$ 和 $B(t)$ 是可微矩阵函数。证明 $\frac{d}{dt}(A(t)B(t)) = A'(t)B(t) + A(t)B'(t)$。**
   ??? success "参考答案"
       直接按矩阵乘法的元素定义 $(AB)_{ij} = \sum a_{ik}b_{kj}$ 进行标量求导并利用标量乘积法则，再重新组合为矩阵乘法即得。绝对不能写成 $A'B + B'A$，因为矩阵一般不交换。

2. **[微积分] 如果 $A(t)$ 总是可逆的，计算 $\frac{d}{dt}(A^{-1}(t))$。**
   ??? success "参考答案"
       由 $A(t)A^{-1}(t) = I$，两边求导得 $A'A^{-1} + A(A^{-1})' = 0$。左乘 $A^{-1}$ 得到 $(A^{-1})' = -A^{-1} A' A^{-1}$。

3. **[微积分] 什么是方向导数和 Fréchet 导数在矩阵分析中的区别？**
   ??? success "参考答案"
       方向导数（Gâteaux 导数）考察沿某个特定矩阵 $H$ 方向的微小变化比率，而 Fréchet 导数则是一个完整的线性映射，能够对任意微小扰动 $H$ 给出最佳线性逼近，要求更强的全局可微性。

4. **[极限] 若 $\lim_{k \to \infty} A^k = O$，证明 $\lim_{k \to \infty} \sum_{i=0}^k A^i = (I-A)^{-1}$。**
   ??? success "参考答案"
       利用等式 $(I-A)(I+A+\dots+A^k) = I - A^{k+1}$。由于 $A^k \to O$，右边趋于 $I$。这正是 Neumann 级数定理。

5. **[收敛] 证明对于任意诱导矩阵范数 $\|A\|$，$\rho(A) \le \|A\|$。**
   ??? success "参考答案"
       设 $A\mathbf{x} = \lambda\mathbf{x}$ 且 $\mathbf{x} \neq \mathbf{0}$。则 $|\lambda|\|\mathbf{x}\| = \|\lambda\mathbf{x}\| = \|A\mathbf{x}\| \le \|A\|\|\mathbf{x}\|$。消去 $\|\mathbf{x}\|$ 得 $|\lambda| \le \|A\|$，对所有特征值成立，故 $\rho(A) \le \|A\|$。

6. **[极限] 为什么马尔可夫链的转移矩阵 $P$ 满足 $\lim_{k \to \infty} P^k$ 总是存在，尽管 $\rho(P) = 1$（并不严格小于 1）？**
   ??? success "参考答案"
       因为转移矩阵 $P$ 在不可约且非周期时，谱半径 1 是代数单根且为唯一的模长最大特征值（Perron-Frobenius定理）。它的 Jordan 形中对应 $\lambda=1$ 的只是 $1 \times 1$ 块，其余特征值模长均严格小于 1，所以极限存在并趋于稳态分布的秩 1 投影。

7. **[矩阵不等式] 如果 $A, B$ 是半正定矩阵，证明 $\operatorname{tr}(AB) \ge 0$。**
   ??? success "参考答案"
       $A$ 半正定，故可写为 $A = C^*C$。则 $\operatorname{tr}(AB) = \operatorname{tr}(C^*CB) = \operatorname{tr}(CBC^*)$。由于 $B$ 半正定，对于任意列向量 $\mathbf{x}$ 有 $\mathbf{x}^*B\mathbf{x} \ge 0$，因此 $CBC^*$ 的对角线元素全非负，迹必非负。

8. **[Gelfand 公式] 简述 Gelfand 公式的核心思想 $\rho(A) = \lim_{k\to\infty} \|A^k\|^{1/k}$。**
   ??? success "参考答案"
       它极其深刻地指出，无论我们在有限步骤内选择什么“畸形”的矩阵范数（这会带来短暂放大），但在无限次幂的渐进行为中，范数的增长率完全由特征值的“最高频率”（谱半径）绝对主导。

9. **[张量导数] $\frac{\partial}{\partial X} \operatorname{tr}(AXB)$ 的结果是什么？**
   ??? success "参考答案"
       结果是 $(BA)^T$。利用迹的循环性质 $\operatorname{tr}(AXB) = \operatorname{tr}(BAX)$，根据 Frobenius 内积定义即得。

10. **[爱因斯坦思考题] 为什么我们在连续介质力学和广义相对论中，总是大量计算矩阵（张量）的迹（Trace）、行列式（Determinant）的导数？这在几何上意味着什么？**
    ??? success "参考答案"
        迹（Trace）代表了空间体积的相对膨胀率（散度 $\nabla \cdot \mathbf{v}$）；行列式（Determinant）代表了绝对体积元。对其求导，本质上是在计算物质或时空在受到外力/引力扰动时的体积变形与压缩，这构成了能量-动量张量守恒律和爱因斯坦场方程的基础。

## 本章小结

本章将微积分的核心概念推广到了矩阵空间，主要内容包括：

1. **矩阵的导数与积分**：确立了逐元素求导的基本法则，以及乘积、逆矩阵求导时的非交换性要点。
2. **标量函数的矩阵导数**：基于 Frobenius 内积给出了迹函数和行列式的梯度计算公式，这是优化理论与机器学习反向传播的基础。
3. **方向导数与 Fréchet 导数**：为非线性矩阵函数（如平方根、指数）提供了严格的线性逼近工具。
4. **矩阵极限与幂级数**：明确了矩阵序列收敛的本质条件，并通过 Gelfand 公式确立了谱半径 $\rho(A)$ 在渐进行为中的绝对统治地位。
5. **Kronecker 积与 Vec 算子**：引入了将矩阵方程转化为普通向量方程的高级张量运算工具。
