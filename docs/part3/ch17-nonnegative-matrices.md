...
1. **[概念] 一个矩阵 $A > O$ 意味着它一定正定吗？**
   ??? success "参考答案"
       不一定。元素全大于零（$A > O$）和特征值全大于零（正定 $A \succ 0$）是完全不同的概念。例如 $\begin{pmatrix} 1 & 10 \\ 10 & 1 \end{pmatrix} > O$，但其特征值为 11 和 -9，是不定的。

2. **[图论] 如果一个非负矩阵 $A$ 是不可约的，它的关联有向图具有什么性质？**
   ??? success "参考答案"
       强连通（Strongly Connected）。即从任意一个节点出发，必定有一条有向路径可以到达任何其他节点。

3. **[谱半径] Perron-Frobenius 定理关于不可约非负矩阵的最大特征值（Perron 根）有什么断言？**
   ??? success "参考答案"
       它断言谱半径 $\rho(A)$ 本身必定是矩阵的一个特征值，且代数重数（和几何重数）严格为 1，同时对应的特征向量的每一个分量都严格为正。

4. **[本原阵] 本原矩阵与一般的不可约矩阵有什么区别？**
   ??? success "参考答案"
       本原矩阵要求循环指数 $h=1$。这意味着除了 $\rho(A)$ 外，复平面圆周 $|\lambda| = \rho(A)$ 上没有其他的特征值。从图论上看，回路长度的最大公约数为 1。它保证了 $A^k$ 极限的存在而不只是周期振荡。

5. **[随机矩阵] 列随机矩阵的最大特征值是多少？为什么？**
   ??? success "参考答案"
       是 1。因为列随机矩阵 $P$ 满足 $\mathbf{1}^T P = \mathbf{1}^T$，说明 $\mathbf{1}$ 是 $P^T$ 对应特征值 1 的特征向量。由于 $\rho(P) \ge 1$ 且由 Gerschgorin 圆盘定理所有特征值模长 $\le 1$，故最大特征值精确为 1。

6. **[极限] 为什么马尔可夫链的稳态分布可以被视为矩阵不断相乘的结果？**
   ??? success "参考答案"
       因为初始分布向量 $\mathbf{p}_0$ 不断左乘转移矩阵 $P$（即 $\mathbf{p}_0^T P^k$），当 $P$ 是本原矩阵时，次大特征值严格小于 1，所以所有其它模态呈指数级衰减，最终收敛到最大特征值 1 对应的左特征向量，也就是稳态分布。

7. **[Birkhoff定理] $n \times n$ 的双随机矩阵构成一个几何体，它的“顶点”是什么？**
   ??? success "参考答案"
       所有的 $n \times n$ 置换矩阵（共有 $n!$ 个）。这就是著名的 Birkhoff-von Neumann 定理。

8. **[图论] 如果不可约非负矩阵的图是一条包含所有节点的单向大环，它的谱有什么特征？**
   ??? success "参考答案"
       它的循环指数 $h = n$。其 $n$ 个特征值完全均匀地分布在复平面上位半径为 $\rho(A)$ 的圆周上（即乘以各个 $n$ 次单位根）。

9. **[PageRank] 为什么 Google 矩阵在构造时要强行加入一项 $(1-\alpha)\frac{1}{n}\mathbf{1}\mathbf{1}^T$？**
   ??? success "参考答案"
       这称为“随机阻尼/悬退”。真实网页图可能存在悬挂节点（无出链）或孤立的子图（破坏不可约性）。加入这一项不仅强行使得图变得全连通（变成了严格正矩阵，保证了唯一的 Perron 向量），还通过 $\alpha$ 调节了收敛速度（次大特征值限制在 $\alpha$ 内）。

10. **[爱因斯坦思考题] 统计力学中假设气体分子在相空间中的运动满足“遍历假说”（Ergodic Hypothesis）。这在有限状态非负矩阵的演化中，对应于哪个数学概念？它向我们保证了什么？**
    ??? success "参考答案"
        它对应于矩阵的**不可约性（Irreducibility）**。不可约性保证了系统的演化网络是强连通的，即随时间推移，系统可以从任何初始状态达到任何其他状态（“走遍整个相空间”）。这确保了系统拥有唯一、宏观且确定的热力学平衡态（唯一的 Perron 稳态），而不受初始局部扰动的影响。

## 本章小结

本章将线性代数与组合图论及概率过程深度结合，主要内容包括：

1. **非负性与图论**：矩阵元素的非负性天然对应于网络连通的转移权重。不可约性等价于图的强连通性。
2. **Perron-Frobenius 定理**：非负矩阵理论的皇冠，严格证明了正矩阵和不可约非负矩阵在谱上的极度规律性（最大实特征值及其正特征向量的存在和唯一性）。
3. **本原性与极限收敛**：阐明了当循环指数为 1 时，矩阵高次幂逼近于秩一投影的渐进行为，奠定了动态网络演化的理论基础。
4. **随机矩阵与马尔可夫链**：应用 PF 定理精确解释了随机漫步过程如何“遗忘初始状态”并收敛至全系统的唯一平稳分布。
5. **PageRank 与 Birkhoff 定理**：展示了非负矩阵在现代互联网搜索引擎及凸几何（双随机矩阵的凸包极点）中的非凡威力。
