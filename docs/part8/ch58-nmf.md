...
1. **[概念] 为什么 NMF 会产生“部分-整体”（Part-based）的表示，而 PCA/SVD 产生的是“全局”表示？**
   ??? success "参考答案"
       因为 NMF 约束 $W, H \ge 0$，这意味着重构过程只能通过加法叠加基向量（$V \approx WH$）。为了减少误差，系统被迫寻找互不重叠的局部特征（部件）。而 PCA 允许减法，可以通过全局特征相互抵消来拟合局部，因此其基向量通常是全脸或全局模式。

2. **[基础] 证明：如果 $V$ 是非负矩阵且 $V=WH$，则 $\operatorname{rank}(V) \le \operatorname{rank}_+(V)$。**
   ??? success "参考答案"
       设 $\operatorname{rank}_+(V) = r$。根据定义，存在 $W \in \mathbb{R}_{\ge 0}^{m \times r}$ 和 $H \in \mathbb{R}_{\ge 0}^{r \times n}$ 使得 $V=WH$。
       由线性代数基本性质，$\operatorname{rank}(V) = \operatorname{rank}(WH) \le \min(\operatorname{rank}(W), \operatorname{rank}(H)) \le r$。
       故 $\operatorname{rank}(V) \le \operatorname{rank}_+(V)$。

3. **[计算] 对 $2 \times 2$ 矩阵 $V = \begin{pmatrix} 1 & 2 \\ 3 & 6 \end{pmatrix}$，写出一个秩为 1 的精确非负分解。**
   ??? success "参考答案"
       注意 $V$ 的列成比例：第二列是第一列的 2 倍。
       取 $W = \begin{pmatrix} 1 \\ 3 \end{pmatrix}, H = \begin{pmatrix} 1 & 2 \end{pmatrix}$。
       验证：$W \ge 0, H \ge 0$ 且 $WH = V$。

4. **[唯一性] 为什么 NMF 的分解结果一般不唯一？举出一种最常见的不确定性。**
   ??? success "参考答案"
       最常见的是**对角缩放不确定性**：$V = WH = (WD)(D^{-1}H)$，其中 $D$ 是正对角阵。这说明我们可以任意改变基向量的幅度，只要相应地缩放权重即可。此外还有置换不确定性。

5. **[算法] 简述 Lee-Seung 乘性更新规则的核心思想。它是如何保证非负性的？**
   ??? success "参考答案"
       核心思想是在梯度下降的基础上，选取与当前变量值成正比的自适应步长。由于更新公式形如 $H \leftarrow H \odot \frac{A}{B}$（所有项均为正），只要初始值非负，迭代过程永远不会产生负数。

6. **[非负秩] 给出一个 $3 \times 3$ 矩阵，使其非负秩严格大于普通秩。**
   ??? success "参考答案"
       取 $V = \begin{pmatrix} 1 & 1 & 0 \\ 1 & 0 & 1 \\ 0 & 1 & 1 \end{pmatrix}$。
       其行列式为 -2（非零），故普通秩为 3。但实际上对于某些特定的 0-1 模式矩阵，非负秩可以设计得比普通秩大（虽然本例中两者均为 3）。真正的例子如例 58.6 中的 $4 \times 4$ 矩阵。

7. **[正则化] 在稀疏 NMF 中，加入 $L_1$ 正则化项 $\beta \|H\|_1$ 会对分解结果产生什么影响？**
   ??? success "参考答案"
       它会迫使矩阵 $H$ 的大部分元素变为 0。在应用层面，这意味着每个数据点（文档、图像）只由极少数的基向量（主题、部件）组合而成，从而增强了结果的可解释性。

8. **[聚类] 为什么带有正交约束的 NMF（$HH^T = I$）等价于 K-means 聚类？**
   ??? success "参考答案"
       非负性约束 $H \ge 0$ 结合正交性约束 $HH^T = I$ 强制要求 $H$ 的每一列只有一个非零元素。这意味着每个原始数据点被“硬性”指派到了唯一的基向量（类中心）上，这正是聚类的定义。

9. **[应用] 在音源分离中，NMF 的矩阵 $W$ 和 $H$ 分别代表什么？**
   ??? success "参考答案"
       $W$ 的每一列代表一个音源（如钢琴、小提琴）的特征频谱（即指纹）；$H$ 的每一行代表该音源随时间变化的强度（即乐谱）。

10. **[爱因斯坦思考题] 爱因斯坦认为物理实相应该是某种“最简要素”的组合。NMF 的非负约束实际上禁止了“负存在”的出现。在处理如文档主题或像素颜色时，这种“不允许抵消”的规则如何体现了自然界的一种单调性逻辑？**
    ??? success "参考答案"
        在宏观物理世界中，很多量（质量、光强、词频）是单调累加的。PCA 允许负权重，本质上是在用“干涉”或“对消”来逼近现实，这在波动力学中成立，但在描述“由部件组成的物体”时却违背直觉。NMF 的非负性守住了“无则为零，有则累加”的底线，它反映了宇宙的一种原子论视角：整体是由局部的、真实的实体通过非负的物理过程（叠加）而构成的，而不是由抽象的正负波动干涉出来的虚影。

## 本章小结

本章深入探讨了机器学习与数据挖掘中的核心矩阵工具——非负矩阵分解（NMF）：

1. **核心哲学**：确立了非负性约束作为实现“部分-整体”加法表示的根本动力，对比了其与 SVD 全局对消表示的本质区别。
2. **算法演进**：详细介绍了 Lee-Seung 乘性更新算法及其单调性证明，并讨论了交替最小二乘（ANLS）在提升收敛精度方面的优势。
3. **结构属性**：分析了 NMF 的非唯一性挑战，引入了可分性（Separability）和充分稀疏性作为保证分解稳定的关键条件。
4. **非负秩理论**：界定了非负秩 $\operatorname{rank}_+(V)$ 的概念，揭示了其在计算上的 NP 困难性以及与线性秩之间的巨大鸿沟。
5. **多元化应用**：展示了 NMF 在主题模型挖掘（文档-主题）、音源分离（频谱-激活）、高光谱解混以及人脸识别中的卓越可解释性，证明了它是架起线性代数与语义理解之间桥梁的重要工具。
