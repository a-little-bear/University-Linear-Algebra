...
****：确立了 $e^{At}$ 作为一阶常系数齐次系统 $\mathbf{x}'=A\mathbf{x}$ 统一解算子的地位，并推导了非齐次系统的杜哈梅积分公式。

****：通过分析 $2 \times 2$ 矩阵特征值的实虚部符号，对动力系统的平衡点（结点、焦点、鞍点、中心）进行了几何分类。对于非线性系统，雅可比矩阵在平衡点处的特征值决定了系统的局部渐近稳定性。

****：展示了如何通过状态变量的扩充，将任意高阶常系数标量方程降阶打击为一阶系统，建立了特征多项式与友矩阵特征值的对应关系。

****：将有限维对称矩阵的谱定理推广到无穷维的微分算子，证明了在此框架下的特征函数必然构成一组完备的正交基。

****：利用微分算子的本征展开，将偏微分方程（如热传导和波动方程）巧妙地转化为无数个彼此独立的常微分方程，彰显了线性叠加原理在物理学中的无上威力。

****：处理了周期系数的微分系统，将参数共振和能带理论的机制归结为单值矩阵特征值（Floquet乘子）的分析。
