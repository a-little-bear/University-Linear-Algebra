...
1. **[协方差] 样本协方差矩阵 $S$ 为什么总是半正定的？**
   ??? success "参考答案"
       因为 $S = \frac{1}{n-1} X_c^T X_c$，任何形如 $A^TA$ 的矩阵必定是半正定的（对于任何向量 $\mathbf{v}$，$\mathbf{v}^T(A^TA)\mathbf{v} = \|A\mathbf{v}\|^2 \ge 0$）。

2. **[多元高斯] 多元高斯分布的等概率密度面在几何上是什么形状？其轴的方向和长度由什么决定？**
   ??? success "参考答案"
       是超椭球面。椭球主轴的方向由协方差矩阵 $\Sigma$ 的正交特征向量决定，而各半轴的长短则直接与特征值的平方根成正比。

3. **[马氏距离] 马哈拉诺比斯距离（Mahalanobis Distance）与普通欧氏距离有何区别？**
   ??? success "参考答案"
       马氏距离通过插入协方差矩阵的逆（$\Sigma^{-1}$）作为度量张量，巧妙地消除了数据各维度的量纲差异和内在相关性，在几何上等于把原数据的倾斜椭球等高面做了一次白化拉伸，变回了标准的球面对称。

4. **[PCA] 主成分分析（PCA）寻找第一主成分时的数学目标是什么？**
   ??? success "参考答案"
       寻找一个单位方向向量，使得数据点投影到该方向上的方差最大化。在代数上，这等价于求解样本协方差矩阵最大特征值所对应的特征向量（即瑞利商的最大化）。

5. **[SVD与PCA] 在计算高维数据的 PCA 时，为什么工业界直接对中心化数据矩阵 $X_c$ 做 SVD，而不是先计算协方差矩阵 $X_c^TX_c$ 再求特征值？**
   ??? success "参考答案"
       因为显式计算 $X_c^TX_c$ 会使得矩阵的条件数被平方（$\kappa(A^TA) = \kappa(A)^2$）。如果原数据较病态，平方后会引发极其严重的浮点数舍入误差和信息丢失；直接对 $X_c$ 施加 SVD 数值稳定性极高。

6. **[核技巧] 机器学习中的核技巧（Kernel Trick）如何在高维甚至无穷维特征空间中计算内积，而无需显式地把数据映射过去？**
   ??? success "参考答案"
       通过选择一个正定核函数 $K(\mathbf{x}, \mathbf{y}) = \langle \phi(\mathbf{x}), \phi(\mathbf{y}) \rangle$。Mercer 定理保证了只要核函数对应的 Gram 矩阵半正定，我们就可以直接在低维原空间评估 $K$，其代数结果完美等效于在极其高维空间中完成了内积运算。

7. **[深度学习] 在神经网络的反向传播（Backpropagation）算法中，误差梯度的跨层传递，本质上是什么线性代数运算？**
   ??? success "参考答案"
       是一连串的**雅可比矩阵**（或其转置，即后一层权重矩阵的转置 $W^T$）与残差向量的连续左乘。本质是多元微积分的链式法则的矩阵表述。

8. **[神经网络] 为什么在数十层的深层神经网络中，如果权重矩阵的最大奇异值长期大于 1，会导致灾难性后果？**
   ??? success "参考答案"
       会导致梯度爆炸。奇异值大于 1 代表该层的线性映射会沿着某些特征方向去拉伸误差信号，经过数十次的指数级连乘后，数值会迅速溢出，导致网络训练直接崩溃。

9. **[推荐系统] 在矩阵分解推荐模型（如 NMF 或 ALS）中，用户隐矩阵 $U$ 和物品隐矩阵 $V$ 的内积 $U V^T$ 代表了什么？**
   ??? success "参考答案"
       代表了算法对整个庞大且极度稀疏的原始用户-物品评分矩阵 $R$ 所作出的一个平滑的**低秩逼近补全**，从而预测出用户尚未点击的物品评分。

10. **[爱因斯坦思考题] “维度灾难”导致在高维空间中任何两点的距离都趋于相等。然而，主成分分析（PCA）和流形学习等线性代数工具奇迹般地挽救了 AI。这说明了我们所处现实世界的数据具有什么内禀属性？**
    ??? success "参考答案"
        这深刻地说明：宇宙中看似杂乱无章、由数百万像素或海量传感器构成的真实物理数据（如人脸、语言大模型），**绝不是均匀地散布在整个高维相空间中的**。它们就像是一张极薄的纸（极低维的流形或低秩子空间），被随机扭曲并空旷地嵌套在高维空间里。线性代数的投影与分解，就是帮我们精准剥离维度虚妄、识破这层“薄纸”几何朝向的神级透视眼。
