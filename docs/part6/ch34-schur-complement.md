...
1. **[概念] 为什么我们把 $D - CA^{-1}B$ 叫做 $D$ 关于 $A$ 的 Schur 补？**
   ??? success "参考答案"
       因为它是在分块矩阵中通过 $A$ 消除 $C$ 后，在原 $D$ 的位置上剩下的部分。它补偿了因为消去 $A$ 和 $C$ 而对 $D$ 造成的代数影响。

2. **[行列式] 如果 $\det(A) = 2$ 且 $\det(M/A) = 5$，那么分块矩阵 $M$ 的行列式是多少？**
   ??? success "参考答案"
       $\det(M) = \det(A) \det(M/A) = 2 \times 5 = 10$。

3. **[正定性] 设 $M = \begin{pmatrix} A & B \\ B^H & D \end{pmatrix}$。若 $A$ 是正定的，且 $M/A$ 也是正定的，证明 $M$ 必定正定。**
   ??? success "参考答案"
       利用块 LDU 分解：$M = L \operatorname{diag}(A, M/A) L^H$。由于 $L$ 可逆，$M$ 的特征值正负号与对角块 $A$ 和 $M/A$ 一致。若两者皆正定，则 $M$ 正定。

4. **[Woodbury] Sherman-Morrison 公式处理的是哪种类型的扰动？**
   ??? success "参考答案"
       处理的是**低秩扰动**（尤其是秩为 1 的扰动）。它允许我们在已知 $A^{-1}$ 的情况下，快速计算出 $(A + uv^H)^{-1}$，而无需重新进行耗时的全矩阵求逆。

5. **[统计] 多元正态分布中，条件协方差矩阵与 Schur 补有什么关系？**
   ??? success "参考答案"
       条件协方差矩阵**精确地等于**联合协方差矩阵关于已知变量块的 Schur 补。这说明 Schur 补捕捉了扣除已知信息后剩余的不确定性。

6. **[计算] 求 $M = \begin{pmatrix} 1 & 2 \\ 2 & 5 \end{pmatrix}$ 关于 $A = (1)$ 的 Schur 补。**
   ??? success "参考答案"
       $M/A = 5 - 2(1)^{-1}2 = 5 - 4 = 1$。

7. **[惯性] Haynsworth 惯性公式 $In(M) = In(A) + In(M/A)$ 有什么实际用途？**
   ??? success "参考答案"
       它允许我们通过分块递归地计算大型对称矩阵的正、负特征值个数，而不需要显式求解所有特征值。这在稳定性分析和二次型分类中非常有用。

8. **[优化] Schur 补引理如何将非线性矩阵不等式转化为线性矩阵不等式（LMI）？**
   ??? success "参考答案"
       它指出 $D - B^H A^{-1} B \succ 0$（含逆矩阵，是非线性的）等价于全矩阵 $\begin{pmatrix} A & B \\ B^H & D \end{pmatrix} \succ 0$（这是关于矩阵元素的线性约束）。这使得半定规划（SDP）能够处理原本复杂的倒数约束。

9. **[商性质] 解释 Schur 补的“商性质”：$(M/A) / (\tilde{A}/A) = M/\tilde{A}$。**
   ??? success "参考答案"
       这说明 Schur 补操作满足类似于算术除法的规律。连续的局部消元与一次性的整体消元是等价的，体现了代数运算的相容性。

10. **[爱因斯坦思考题] 在爱因斯坦的引力场方程中，由于时空的非线性和分块结构，我们经常需要计算度规张量的逆。为什么说 Schur 补是处理“局部物理规律”与“背景时空”相互作用的理想工具？**
    ??? success "参考答案"
        Schur 补本质上是一种“有效作用量”的提取。当我们关注子系统 $D$ 而将背景 $A$ 视为已知或需要消去的自由度时，Schur 补 $M/A$ 正好描述了子系统在考虑了与背景的相互作用后所表现出的“有效响应”。它是从整体关联中剥离出局部独立物理规律的数学手术刀。

## 本章小结

本章系统探讨了分块矩阵理论中最重要的分析工具——Schur 补，主要内容包括：

1. **定义与块消元**：确立了 Schur 补作为块高斯消元剩余项的地位，统一了 $M/A$ 和 $M/D$ 的表达形式。
2. **行列式与求逆**：推导了基于 Schur 补的行列式积公式和块求逆公式，引出了著名的 Sherman-Morrison-Woodbury 矩阵更新恒等式。
3. **正定性与惯性**：证明了分块 Hermite 矩阵正定性的 Schur 补判据，并介绍了 Haynsworth 惯性加法公式。
4. **统计与优化应用**：展示了 Schur 补在刻画条件正态分布和构造线性矩阵不等式（LMI）中的决定性作用。
5. **代数结构**：深入分析了 Schur 补的商性质和单调性，揭示了其作为一种算子运算的内在美感。
