...
!!! example "例 38A.E1"
    证明：若 $A$ 是 Z-矩阵且 $A + A^T$ 正定，则 $A$ 是非奇异 M-矩阵。

    **提示**：$A + A^T$ 正定意味着 $A$ 的所有特征值具有正实部。

!!! example "例 38A.E2"
    设 $A$ 是 $n \times n$ 非奇异 M-矩阵。证明 $\det(A) \le \prod_{i=1}^n a_{ii}$，等号成立当且仅当 $A$ 是对角矩阵。

    **提示**：利用 Fischer 不等式或 Hadamard 不等式。

!!! example "例 38A.E3"
    设 $A$ 是非奇异 M-矩阵，$0 \le D \le A$ 是对角矩阵（对角元素 $0 \le d_{ii} \le a_{ii}$）。证明 $A - D$ 不一定是 M-矩阵，但 $A$ 的对角元素全部被替换为更大的正数后仍为 M-矩阵。

!!! example "例 38A.E4"
    设 $A$ 为 $3 \times 3$ 不可约非奇异 M-矩阵。

    (a) 计算 $A^{-1}$ 并验证 $A^{-1} > 0$。

    (b) 对 $A = \begin{pmatrix} 3 & -1 & -1 \\ -1 & 3 & -1 \\ -1 & -1 & 3 \end{pmatrix}$，计算 Jacobi 和 Gauss-Seidel 迭代矩阵的谱半径，验证 Stein-Rosenberg 定理。

!!! example "例 38A.E5"
    设 $Q$ 为不可约连续时间马尔可夫链的生成元矩阵（$q_{ij} \ge 0$ 对 $i \ne j$，$Q\mathbf{1} = 0$）。

    (a) 证明 $-Q$ 是不可约奇异 M-矩阵。

    (b) 证明 $-Q$ 的任何 $(n-1) \times (n-1)$ 主子矩阵是非奇异 M-矩阵。

    (c) 利用 (b) 解释为什么通过删去 $Q$ 的一行一列可以求解平稳分布 $\pi$。

!!! example "例 38A.E6"
    （Schur 补）设 $A = \begin{pmatrix} 4 & -1 & -1 \\ -2 & 5 & -1 \\ -1 & -1 & 4 \end{pmatrix}$。

    (a) 验证 $A$ 是 M-矩阵。

    (b) 计算 $A/A_{11}$（其中 $A_{11} = \begin{pmatrix} 4 & -1 \\ -2 & 5 \end{pmatrix}$），验证它是 M-矩阵。

!!! example "例 38A.E7"
    设 $A, B$ 为 $2 \times 2$ 非奇异 M-矩阵。计算 Fan 积 $A \star B$ 并验证它是 M-矩阵。

!!! example "例 38A.E8"
    （Varga 比较定理的应用）对 $A = \begin{pmatrix} 4 & -1 & 0 \\ -1 & 4 & -1 \\ 0 & -1 & 4 \end{pmatrix}$，

    (a) 计算 Jacobi 分裂和 Gauss-Seidel 分裂的迭代矩阵及谱半径。

    (b) 验证 Varga 比较定理（$N_{Jacobi} \ge N_{GS}$ 意味着 $\rho(T_{GS}) \le \rho(T_J)$）。

    (c) 对 $\omega = 1.1$，计算 SOR 迭代矩阵的谱半径。

!!! example "例 38A.E9"
    （Hawkins-Simon 条件）三部门经济的投入系数矩阵为
    $$C = \begin{pmatrix} 0.3 & 0.2 & 0.1 \\ 0.2 & 0.3 & 0.2 \\ 0.1 & 0.1 & 0.3 \end{pmatrix}.$$

    (a) 检验 Hawkins-Simon 条件。

    (b) 若最终需求为 $d = (100, 150, 80)^T$，求总产出 $x$。

    (c) 计算 Leontief 乘数矩阵 $(I-C)^{-1}$ 并解释经济含义。

!!! example "例 38A.E10"
    证明：若 $A$ 是非奇异 M-矩阵，则 $A^{-1}$ 的最大元素出现在对角线上，即 $(A^{-1})_{ii} \ge (A^{-1})_{ij}$ 对所有 $i, j$。

    **提示**：利用 $A^{-1} = s^{-1}\sum(s^{-1}B)^k$ 和对角线路径的贡献。

!!! example "例 38A.E11"
    （弱正则分裂）构造一个分裂 $A = M - N$，其中 $M^{-1} \ge 0$，$M^{-1}N \ge 0$，但 $N$ 不是非负矩阵。验证定理 38A.16 对此例成立。

!!! example "例 38A.E12"
    证明：非奇异 M-矩阵 $A$ 满足 $A + A^T$ 正定当且仅当 $A$ 是正定的（对称 M-矩阵的情形）。对非对称 M-矩阵给出反例说明 $A + A^T$ 可以不正定。
