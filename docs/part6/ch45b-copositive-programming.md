...
1. **[基础] 判定 $A = \begin{pmatrix} 1 & -1 \\ -1 & 2 \end{pmatrix}$ 是否余正。**
   ??? success "参考答案"
       由于 $A$ 是正定的（特征值约为 0.38 和 2.62），故 $x^T A x \ge 0$ 对所有向量 $x$ 成立，当然也对所有 $x \ge 0$ 成立。因此 $A$ 是余正的，尽管它含有负元素。

2. **[对偶性] 证明完全正锥 $\mathcal{CP}_n$ 的对偶锥是余正锥 $\mathcal{COP}_n$。**
   ??? success "参考答案"
       $\mathcal{CP}_n$ 的生成元是 $xx^T$（$x \ge 0$）。根据对偶锥定义，$C \in \mathcal{CP}_n^*$ 当且仅当对所有生成元有 $\langle xx^T, C \rangle = x^T C x \ge 0$ 对所有 $x \ge 0$。这恰好是余正矩阵的定义。

3. **[复杂度] 判定一个矩阵是否余正的计算复杂度是多少？**
   ??? success "参考答案"
       这是 **co-NP 完全**问题。这反映了余正规划能够精确编码各种 NP-hard 组合优化问题的本质困难性。

4. **[独立数] 将图的独立数 $\alpha(G)$ 表示为余正规划。**
   ??? success "参考答案"
       $\frac{1}{\alpha(G)} = \min \{ t : tI + A_G \in \mathcal{COP}_n \}$，其中 $A_G$ 是邻接矩阵。该公式给出了独立数的精确值，而非像 SDP 松弛那样仅给出上界。

5. **[小维度] 为什么当 $n \le 4$ 时 $\mathcal{COP}_n = \mathcal{S}_n^+ + \mathcal{N}_n$？**
   ??? success "参考答案"
       在低维度下，非负正交锥的几何结构相对简单，任何余正二次型都可以分解为一个半正定项和一个非负项之和。从 $n=5$ 开始，由于五边形（奇圈）结构的出现，这种分解不再总是可行。

6. **[Horn矩阵] 提供一个余正但不属于 $\mathcal{S}_5^+ + \mathcal{N}_5$ 的矩阵示例。**
   ??? success "参考答案"
       经典的例子是 Horn 矩阵 $H$（对角线为 1，次对角线及其对称位置为 -1，其余为 1）。它是余正的，但不能表示为半正定阵与非负阵之和。

7. **[边界] 刻画余正锥的边界。**
   ??? success "参考答案"
       矩阵 $A$ 位于 $\mathcal{COP}_n$ 的边界上，当且仅当存在非零向量 $x \ge 0$ 使得 $x^T A x = 0$。

8. **[MAX-CUT] MAX-CUT 问题能否精确表述为余正规划？**
   ??? success "参考答案"
       是的。所有标准的 NP-hard 问题，包括 MAX-CUT、图着色和最大团问题，都可以通过完全正锥或余正锥上的线性规划给出非松弛的精确表述。

9. **[SOS层次] 简述 Parrilo 的 SOS 层次逼近。**
   ??? success "参考答案"
       该方法利用一系列嵌套的、由和数平方（SOS）条件定义的锥 $\mathcal{K}_n^{(r)}$ 来逼近 $\mathcal{COP}_n$。随着阶数 $r$ 的增加，这些可计算的 SDP 约束将收敛于余正锥。

10. **[LCP] 余正矩阵与线性互补问题（LCP）有何联系？**
    ??? success "参考答案"
        若 $M$ 是余正矩阵，则只要 LCP$(M, q)$ 问题的可行域非空，就保证该问题一定有解。
