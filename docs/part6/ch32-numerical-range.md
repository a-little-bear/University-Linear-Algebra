...
1. **[基础] 设 $A = \operatorname{diag}(1, i)$。计算它的数值域 $W(A)$。**
   ??? success "参考答案"
       根据定理 32.4（正规矩阵的数值域是特征值的凸包），$W(A) = \operatorname{conv}\{1, i\}$。在复平面上，这是连接点 $(1, 0)$ 和 $(0, i)$ 的一条**闭线段**。

2. **[ nilpotent ] 计算 $2 \times 2$ 幂零矩阵 $A = \begin{pmatrix} 0 & 1 \\ 0 & 0 \end{pmatrix}$ 的数值域。**
   ??? success "参考答案"
       由例 32.1 的推导可知，$W(A)$ 是一个以原点为中心、半径为 $1/2$ 的**闭圆盘**。注意它的特征值只有 $\{0\}$，但数值域却覆盖了整个圆盘。

3. **[包含性] 证明：任何矩阵 $A$ 的特征值 $\lambda$ 必定属于数值域 $W(A)$。**
   ??? success "参考答案"
       设 $\mathbf{x}$ 是对应于 $\lambda$ 的单位特征向量。则 $\mathbf{x}^* A \mathbf{x} = \mathbf{x}^* (\lambda \mathbf{x}) = \lambda (\mathbf{x}^* \mathbf{x}) = \lambda \cdot 1 = \lambda$。由定义，$\lambda \in W(A)$。

4. **[正规矩阵] 证明：若 $A$ 是正规矩阵，其数值半径 $w(A)$ 与谱半径 $\rho(A)$ 相等。**
   ??? success "参考答案"
       对于正规矩阵，$W(A) = \operatorname{conv}\{\lambda_i\}$。数值半径是该凸包中点到原点的最大距离，显然等于模最大的特征值的模，即 $\max |\lambda_i| = \rho(A)$。

5. **[不等式] 验证矩阵 $A = \begin{pmatrix} 0 & 2 \\ 0 & 0 \end{pmatrix}$ 满足 $w(A) \le \|A\| \le 2w(A)$。**
   ??? success "参考答案"
       1. 数值半径 $w(A) = 1$（半径为 1 的圆盘）。
       2. 算子范数 $\|A\| = \sigma_1(A) = 2$。
       3. 验证：$1 \le 2 \le 2(1)$。不等式取到了右端的等号极限。

6. **[Hermite] 如果 $W(A)$ 是实轴上的一个区间，能否断定 $A$ 是 Hermite 矩阵？**
   ??? success "参考答案"
       可以。若 $W(A) \subseteq \mathbb{R}$，则对所有单位向量 $\mathbf{x}$，$\mathbf{x}^* A \mathbf{x} = (\mathbf{x}^* A \mathbf{x})^* = \mathbf{x}^* A^* \mathbf{x}$。这意味着二次型 $\mathbf{x}^* (A - A^*) \mathbf{x} = 0$ 恒成立，因此 $A - A^* = 0$，即 $A = A^*$。

7. **[酉矩阵] 证明：若 $U$ 是酉矩阵，其数值域 $W(U)$ 必定包含在复平面的单位圆盘内。**
   ??? success "参考答案"
       因为 $w(U) \le \|U\| = 1$。事实上，$W(U)$ 是位于单位圆周上的特征值的凸包（多边形）。

8. **[子加性] 举例说明 $W(A+B)$ 可能严格小于 $W(A) + W(B)$。**
   ??? success "参考答案"
       取 $A = \begin{pmatrix} 1 & 0 \\ 0 & 0 \end{pmatrix}, B = \begin{pmatrix} -1 & 0 \\ 0 & 0 \end{pmatrix}$。则 $W(A)=[0,1], W(B)=[-1,0]$。$W(A)+W(B)=[-1,1]$。
       但 $A+B = O$，故 $W(A+B) = \{0\}$。

9. **[凸性] 为什么数值域的凸性（Toeplitz-Hausdorff 定理）在算子理论中被认为是非平凡的？**
   ??? success "参考答案"
       因为单位球面本身并不是凸集。一个线性算子在非凸集合上的像通常也不必是凸的。数值域的凸性反映了内积空间中复二次型结构的深层连通性。

10. **[爱因斯坦思考题] 在量子力学中，算子 $A$ 的数值域 $W(A)$ 代表了所有物理上可能的“测量期望值”。为什么“数值域是凸集”这一数学事实，实际上保证了量子状态叠加原理的逻辑自洽？**
    ??? success "参考答案"
        如果数值域不是凸的，那么两个可能状态（对应期望值 $z_1, z_2$）的统计混合（即 $\alpha z_1 + (1-\alpha)z_2$）可能会产生一个物理上无法解释（不在 $W(A)$ 内）的测量预言。Toeplitz-Hausdorff 定理从纯代数层面保证了：无论我们如何通过概率权重混合已有的量子观测结果，所得的平均值在逻辑上永远属于该算子允许的物理量程之内。它是量子统计完备性的几何保障。

## 本章小结

本章将线性算子的谱信息与复平面的拓扑几何联系在一起，主要内容包括：

1. **数值域 $W(A)$ 的定义**：引入了 Rayleigh 商集合的概念，作为特征值在非单位特征向量方向上的广义延伸。
2. **Toeplitz-Hausdorff 定理**：证明了无论矩阵阶数多高，数值域永远是复平面上的凸紧集，这是算子分析的基石。
3. **正规与 Hermite 矩阵的刻画**：明确了正规阵的数值域是特征值的凸包，而 Hermite 阵的数值域退化为实轴区间。
4. **数值半径 $w(A)$**：确立了作为矩阵范数的数值半径，并推导了著名的双侧估计不等式 $w(A) \le \|A\| \le 2w(A)$。
5. **高阶数值域与量子纠错**：探讨了秩-k 数值域的凸性，展示了其在判定量子相干性和构造量子纠错码中的前沿应用。
