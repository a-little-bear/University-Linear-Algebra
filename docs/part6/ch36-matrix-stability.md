...
1. **[基础] 如何仅通过特征值判定一个连续时间系统 $\dot{\mathbf{x}} = A\mathbf{x}$ 是渐近稳定的？**
   ??? success "参考答案"
       矩阵 $A$ 的所有特征值 $\lambda_i$ 的实部必须严格小于 0（即全部位于复平面的左半平面）。这就是 Hurwitz 稳定性。

2. **[离散系统] 对于迭代系统 $\mathbf{x}_{k+1} = A\mathbf{x}_k$，其稳定的特征值判据是什么？**
   ??? success "参考答案"
       矩阵 $A$ 的所有特征值的模长必须严格小于 1（即全部位于复平面的单位圆内部）。这就是 Schur 稳定性。

3. **[Lyapunov] 求解 Lyapunov 方程 $A^T P + PA = -I$。如果解出的 $P$ 是正定的，说明了系统的什么性质？**
   ??? success "参考答案"
       说明系统矩阵 $A$ 是 Hurwitz 稳定的。正定矩阵 $P$ 实际上定义了一个能量函数（Lyapunov 函数） $V(\mathbf{x}) = \mathbf{x}^T P \mathbf{x}$，方程保证了该能量沿系统轨迹严格衰减。

4. **[Routh] 使用 Routh-Hurwitz 准则判定 $p(\lambda) = \lambda^2 + 3\lambda + 2$ 的稳定性。**
   ??? success "参考答案"
       对于二阶多项式，所有系数为正即稳定。此处 $1, 3, 2 > 0$，故稳定。其根为 -1 和 -2，符合判据。

5. **[惯性] 什么是矩阵的惯性（Inertia）三元组？**
   ??? success "参考答案"
       惯性 $In(A) = (n_+, n_-, n_0)$ 分别代表特征值实部为正、为负、为零的个数。对于 Hermite 矩阵，它代表正、负、零特征值的实际个数。

6. **[Sylvester] Sylvester 惯性定律指出，在什么变换下，矩阵的惯性保持不变？**
   ??? success "参考答案"
       在**合同变换**（Congruence transformation）下保持不变，即 $B = S^H A S$（$S$ 可逆）。这说明二次型的本质结构（签名）不随基变换而改变。

7. **[D-稳定性] 为什么在生态学模型中，研究矩阵 $A$ 的 D-稳定性（即 $DA$ 对所有正对角阵 $D$ 都稳定）至关重要？**
   ??? success "参考答案"
       因为 $D$ 通常代表不同物种的内禀增长率。D-稳定性保证了无论物种进化的速率快慢如何（只要是正向增长），生态系统的相互竞争结构（由 $A$ 描述）都能维持整体平衡的稳定性。

8. **[Kharitonov] Kharitonov 定理最神奇的地方在哪里？**
   ??? success "参考答案"
       它指出判定一族系数在区间内波动的无穷多个多项式的鲁棒稳定性，竟然只需要检验其中**四个**特定的“极端”多项式。这极大地简化了不确定系统的稳定性分析。

9. **[临界稳定] 若矩阵 $A$ 的特征值都在左半平面，但有一个特征值正好在虚轴上（$\operatorname{Re}(\lambda)=0$），系统一定稳定吗？**
   ??? success "参考答案"
       不一定。如果该特征值的 Jordan 块大小大于 1（非半单），解会出现 $t, t^2$ 等多项式增长项，导致系统失控。只有当虚轴上的特征值全部半单时，系统才是（非渐近）稳定的。

10. **[爱因斯坦思考题] 爱因斯坦最初认为宇宙是静态且永恒的，但后来的分析显示，即使是带有宇宙学常数的广义相对论静态解，在数学上也是“不稳定平衡”的。如果将宇宙的演化方程线性化为一个矩阵 $A$，这种“不稳定”在特征值上表现为什么？这对宇宙的命运意味着什么？**
    ??? success "参考答案"
        这种不稳定表现为矩阵 $A$ 拥有至少一个**实部大于零**的特征值（正反馈）。这意味着任何微小的涨落（如物质密度的局部不均）都会被指数级放大，而不是被拉回平衡。在物理上，这预示了宇宙不可能永远保持静止：它要么在引力下坍缩，要么在排斥力下永久膨胀。稳定性理论告诉我们，宇宙的本质是动态演化的，静止只是一种代数上的脆弱巧合。

## 本章小结

本章系统论述了线性系统维持平衡与秩序的数学准则，主要内容包括：

1. **稳定性定义**：确立了 Hurwitz 稳定（左半平面）和 Schur 稳定（单位圆内）作为连续与离散系统收敛性的根本判据。
2. **代数判据**：引入了 Routh-Hurwitz 和 Schur-Cohn/Jury 表，实现了无需显式求根即可判定多项式稳定性的算法化。
3. **Lyapunov 方法**：证明了稳定性与矩阵方程（Lyapunov/Stein方程）正定解之间的等价性，为动力系统提供了“能量耗散”的直观图景。
4. **惯性理论**：通过 Sylvester 惯性定律揭示了矩阵谱分布在合同变换下的某种“拓扑不变性”，并利用 Ostrowski-Schneider 定理统一了惯性与稳定性。
5. **鲁棒与数值稳定性**：探讨了 D-稳定性和 Kharitonov 定理在处理系统不确定性时的威力，并利用 Kreiss 矩阵定理界定了数值计算中矩阵幂增长的极限边界。
6. **物理关联**：展示了稳定性分析如何从纯代数技巧升华为解释自然界演化方向（如生态平衡、宇宙膨胀）的核心逻辑工具。
